...
def build_omni_train_test_split(
    run_records: List[Dict[str, Any]],
    patient_index: Dict[str, Dict[str, Any]],
) -> List[Dict[str, List[str]]]:
    indexed_subjects = set(patient_index.keys())
    train_subjects = sorted(
        {
            str(record["subject_id"])
            for record in run_records
            if str(record.get("split", "")).lower() == "train" and str(record["subject_id"]) in indexed_subjects
        }
    )
    test_subjects = sorted(
        {
            str(record["subject_id"])
            for record in run_records
            if str(record.get("split", "")).lower() == "test" and str(record["subject_id"]) in indexed_subjects
        }
    )
    if not train_subjects or not test_subjects:
        raise ValueError(
            "Omni split requires at least one train patient and one test patient after feature extraction. "
            f"Detected train={len(train_subjects)}, test={len(test_subjects)}."
        )
    return [
        {
            "fold_idx": 1,
            ...
            "train_record_splits": ["train"],
            "test_record_splits": ["test"],
        }
    ]
...
def data_provider(args: Any):
    ...
    if str(getattr(args, "split_strategy", "5fold")).lower() == "omni":
        return run_records, patient_index, build_omni_train_test_split(run_records, patient_index)
...
__all__ = ["build_omni_train_test_split", "build_outer_splits", "data_provider", "split_train_val_subjects"]
