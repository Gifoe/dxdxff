...
def _log(message: str) -> None:
    print(f"[TeChEZ-Ranker][Data] {message}", flush=True)


def _use_omni_channel_task(args: Any) -> bool:
    dataset_format = str(getattr(args, "dataset_format", "")).strip().lower()
    split_strategy = str(getattr(args, "split_strategy", "")).strip().lower()
    return bool(getattr(args, "omni_channel_task", False)) or dataset_format == "omni" or split_strategy == "omni"
...
def _cache_signature(args: Any) -> Dict[str, Any]:
    return {
        ...
        "feature_scales_sec": str(getattr(args, "feature_scales_sec", "5,10,20")),
        "raw_temporal_duration_sec": float(getattr(args, "raw_temporal_duration_sec", 5.0)),
        "raw_temporal_sfreq": float(getattr(args, "raw_temporal_sfreq", 256.0)),
        "ez_definition": str(getattr(args, "ez_definition", "soz_or_resected")),
        "spectral_min_freq": float(getattr(args, "spectral_min_freq", 1.0)),
        "spectral_max_freq": float(getattr(args, "spectral_max_freq", 150.0)),
...
def _cache_path(args: Any) -> Path:
    ...
    return cache_dir / f"techez_hybrid_cache_v{CACHE_VERSION}_{cache_key}.pkl"
...
def _extract_run_feature_record_from_payload(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    record = extract_run_feature_record(
        ...
        feature_scales_sec=payload["feature_scales_sec"],
        raw_temporal_duration_sec=payload["raw_temporal_duration_sec"],
...
def build_or_load_run_records(args: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    if _use_omni_channel_task(args):
        from omni_channel_records import build_or_load_run_records as build_or_load_omni_run_records

        return build_or_load_omni_run_records(args)
    ...
    _log(f"Preparing hybrid seizure-sample cache at {cache_path}")
    ...
    if runs_df.empty:
        raise ValueError("No BIDS runs were discovered for the hybrid TeChEZ pipeline.")
    ...
    _log(
        ...
        f"across {runs_df['subject_id'].nunique()} patients. Hybrid feature extraction will use {feature_num_workers} worker(s)."
    )
...
def flatten_window_samples(
    ...
    *,
    ...
    selected_splits = {str(split).lower() for split in splits} if splits is not None else None
    samples: List[Dict[str, Any]] = []

    for run_record in run_records:
        if selected_subjects is not None and run_record["subject_id"] not in selected_subjects:
            continue
        if selected_splits is not None and str(run_record.get("split", "")).lower() not in selected_splits:
            continue
        ...
        samples.append(
            {
                ...
                "split": str(run_record.get("split", "")),
