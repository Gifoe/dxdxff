...
class Exp_EZLocalization(Exp_Basic):
    ...
    def _build_patient_payloads(
        ...
        normalizer,
        *,
        record_splits: Sequence[str] | None = None,
    ) -> List[Dict[str, Any]]:
        seizure_samples = flatten_window_samples(
            self.run_records,
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        for split in self.outer_splits:
            ...
            train_record_splits = split.get("train_record_splits")
            test_record_splits = split.get("test_record_splits")
            ...
            train_seizure_samples = flatten_window_samples(
                self.run_records,
                subject_ids=train_subjects,
                splits=train_record_splits,
            )
            ...
            train_payloads_raw = self._build_patient_payloads(
                train_subjects,
                normalizer,
                record_splits=train_record_splits,
            )
            val_payloads_raw = self._build_patient_payloads(
                val_subjects,
                normalizer,
                record_splits=train_record_splits,
            )
            test_payloads_raw = self._build_patient_payloads(
                test_subjects,
                normalizer,
                record_splits=test_record_splits,
            )
