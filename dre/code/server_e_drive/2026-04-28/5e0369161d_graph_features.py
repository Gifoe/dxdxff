...
def compute_dynamic_adjacency(
    ...
    if method_l in {"pearson", "pearson_abs"}:
        ...
        return _phase_connectivity(data, sfreq, method_l, topk)
    else:
    ...
    return _topk_sparsify(adjacency, topk=topk)
...
def compute_dynamic_graph_features(
    ...
    for start, end in zip(window_set.window_starts, window_set.window_ends):
        ...
        adjacency = compute_dynamic_adjacency(window_data, window_set.sfreq, method=graph_method, topk=topk)
...
def _phase_connectivity(data: np.ndarray, sfreq: float, method: str, topk: int) -> np.ndarray:
    ...
    return _topk_sparsify(_symmetrize_zero_diag(adj), topk=topk)
...
def _topk_sparsify(adjacency: np.ndarray, topk: int) -> np.ndarray:
    c = adjacency.shape[0]
    if topk <= 0 or topk >= c:
