...
from .baseline_normalization import (
    baseline_correct_adjacency,
    robust_baseline_self_comparison,
    robust_baseline_zscore,
)
...
def build_dynamic_seizure_sample(
    ...
    graph_features, adjacency_seq, _ = compute_dynamic_graph_features(
        ...
        topk=cfg.graph_topk,
        adaptive_topk=cfg.graph_use_adaptive_topk,
        edge_density=cfg.graph_edge_density,
        topk_min=cfg.graph_topk_min,
        topk_max=cfg.graph_topk_max,
    )

    feature_parts = []
    short_feature_stack = None
    if cfg.use_short_features:
        ...
        short_feature_stack = np.stack(short_blocks, axis=0).astype(np.float32, copy=False)
        feature_parts.append(short_feature_stack)
    ...
    seizure_quality = _estimate_seizure_quality(short_feature_stack, window_set.baseline_mask, window_set.window_centers_sec)
    if cfg.use_baseline_normalization:
        if cfg.use_self_comparison_features:
            features = robust_baseline_self_comparison(features, window_set.baseline_mask)
        else:
    ...
    return DynamicSeizureSample(
        ...
        center_id=getattr(seizure, "center_id", None),
        patient_id=getattr(seizure, "patient_id", None),
        ...
        seizure_quality=float(seizure_quality),
        edf_path=getattr(seizure, "edf_path", None),
        onset_time=float(seizure.seizure_onset_sec),
        offset_time=float(seizure.seizure_offset_sec) if seizure.seizure_offset_sec is not None else None,
    )
...
def build_dynamic_patient_sample(
    ...
    return DynamicPatientSample(
        ...
        center_id=getattr(patient, "center_id", None),
...
def align_seizure_sample_to_canonical(
    ...
    return DynamicSeizureSample(
        ...
        seizure_quality=float(sample.seizure_quality) * float(channel_mask.mean() if channel_mask.size else 0.0),
        edf_path=sample.edf_path,
        onset_time=sample.onset_time,
        offset_time=sample.offset_time,
    )
...
def collate_dynamic_patient_batch(batch: Sequence[DynamicPatientSample]) -> dict[str, Any]:
    ...
    seizure_quality = torch.zeros((b, s_max), dtype=torch.float32)
    topology_features = torch.zeros((b, c_max, 4), dtype=torch.float32)
    ...
    for batch_idx, patient in enumerate(batch):
        ...
        topology_features[batch_idx, :c, :] = torch.as_tensor(_build_topology_features(patient.channel_meta), dtype=torch.float32)
        subject_ids.append(patient.subject_id)
        center_ids.append(patient.center_id)
        ...
        for seizure_idx, sample in enumerate(patient.seizure_samples):
            ...
            seizure_quality[batch_idx, seizure_idx] = float(sample.seizure_quality)
    ...
    return {
        ...
        "seizure_quality": seizure_quality,
...
def _build_time_encoding(time_sec: np.ndarray) -> np.ndarray:
    ...
    return np.stack(
        [
            ...
            np.sin(np.pi * scaled),
            np.cos(np.pi * scaled),
            np.sin(2.0 * np.pi * scaled),
            np.cos(2.0 * np.pi * scaled),
            np.sin(4.0 * np.pi * scaled),
            np.cos(4.0 * np.pi * scaled),
            ((t >= -10.0) & (t <= -2.0)).astype(np.float32),
            ((t > -2.0) & (t < 0.0)).astype(np.float32),
            ((t >= 0.0) & (t <= 2.0)).astype(np.float32),
            (t > 2.0).astype(np.float32),
        ],
        axis=-1,
...
TIME_FEATURE_NAMES = [
    ...
    "cos_4x",
    "indicator_baseline",
    "indicator_pretransition",
    "indicator_onset",
    "indicator_postonset",
]


def _feature_names(cfg: BNPDGSConfig) -> list[str]:
    base: list[str] = []
    if cfg.use_short_features:
        base.extend(SHORT_FEATURE_NAMES)
    if cfg.use_lowfreq_features:
        base.extend(LOWFREQ_FEATURE_NAMES)
    if cfg.use_graph_features:
        base.extend(GRAPH_FEATURE_NAMES)
    if cfg.use_baseline_normalization and cfg.use_self_comparison_features:
        names = [f"{name}_z" for name in base]
        names.extend(f"{name}_delta" for name in base)
        names.extend(f"{name}_ratio" for name in base)
    elif cfg.use_baseline_normalization:
        names = [f"{name}_z" for name in base]
    else:
        names = list(base)
...
def _estimate_seizure_quality(
    short_features: np.ndarray | None,
    baseline_mask: np.ndarray,
    time_sec: np.ndarray,
) -> float:
    if short_features is None or short_features.ndim != 3 or short_features.shape[-1] <= 10:
        return 1.0
    baseline = np.asarray(baseline_mask, dtype=bool)
    onset = (np.asarray(time_sec) >= 0.0) & (np.asarray(time_sec) <= 2.0)
    if not np.any(baseline) or not np.any(onset):
        return 1.0
    high_gamma_idx = 10
    baseline_hg = np.nanmedian(short_features[baseline, :, high_gamma_idx])
    onset_hg = np.nanmedian(short_features[onset, :, high_gamma_idx])
    delta = float(onset_hg - baseline_hg)
    quality = 0.5 + 1.0 / (1.0 + np.exp(-delta))
    return float(np.clip(quality, 0.25, 1.50))


def _build_topology_features(channel_meta: Sequence[dict]) -> np.ndarray:
    if not channel_meta:
        return np.zeros((0, 4), dtype=np.float32)
    groups = [str(meta.get("contact_group", "")) for meta in channel_meta]
    group_order = {group: idx for idx, group in enumerate(sorted(set(groups)))}
    numbers = np.asarray(
        [
            float(meta.get("contact_number")) if meta.get("contact_number") is not None else np.nan
            for meta in channel_meta
        ],
        dtype=np.float32,
    )
    fallback = np.arange(1, len(channel_meta) + 1, dtype=np.float32)
    numbers = np.where(np.isfinite(numbers), numbers, fallback)
    global_max = float(np.max(numbers)) if numbers.size else 1.0
    num_groups = max(len(group_order), 1)
    features = np.zeros((len(channel_meta), 4), dtype=np.float32)
    for idx, (meta, group, number) in enumerate(zip(channel_meta, groups, numbers)):
        same_group = np.asarray([groups[j] == group for j in range(len(groups))], dtype=bool)
        group_numbers = numbers[same_group]
        group_min = float(np.min(group_numbers)) if group_numbers.size else float(number)
        group_max = float(np.max(group_numbers)) if group_numbers.size else float(number)
        denom = max(group_max - group_min, 1.0)
        features[idx, 0] = float(group_order[group]) / max(num_groups - 1, 1)
        features[idx, 1] = float(number) / max(global_max, 1.0)
        features[idx, 2] = (float(number) - group_min) / denom
        features[idx, 3] = float(same_group.sum()) / max(len(channel_meta), 1)
