...
def robust_baseline_self_comparison(
    features: np.ndarray,
    baseline_mask: np.ndarray,
    eps: float = 1e-5,
) -> np.ndarray:
    ...
    z = (x - median) / (iqr + float(eps))
    delta = x - median
    ratio = np.log((np.clip(x, 0.0, None) + float(eps)) / (np.clip(median, 0.0, None) + float(eps)))
    result = np.concatenate([z, delta, ratio], axis=-1)
    return np.nan_to_num(result, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32, copy=False)
...
__all__ = ["baseline_correct_adjacency", "robust_baseline_self_comparison", "robust_baseline_zscore"]
