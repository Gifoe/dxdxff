...
class CrossSeizureMILAggregator(nn.Module):
    ...
    def forward(
        ...
        seizure_quality: torch.Tensor | None = None,
        ...
        if seizure_quality is not None:
            quality = seizure_quality.to(device=scores.device, dtype=scores.dtype).clamp_min(1e-6)
            scores = scores + torch.log(quality)[:, :, None]
