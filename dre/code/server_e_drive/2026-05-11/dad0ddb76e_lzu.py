    ...
    for row_idx, row in seizure_rows.iterrows():
        ...
        if edf_path is None:
            ...
            detail = {
                ...
                "candidate_paths": ";".join(_lzu_candidate_edf_paths(cfg.lzu_root, subject_name, edf_stem_candidates)),
            }
...
def _resolve_lzu_edf_path(root: Path, subject_name: str, seizure_number: str, edf_index: Mapping[tuple[str, str], Path]) -> Path | None:
    ...
    for candidate in _lzu_candidate_edf_paths(root, subject_name, seizure_stems):
...
def _lzu_candidate_edf_paths(root: Path, subject_name: str, seizure_stems: list[str]) -> list[str]:
    paths: list[str] = []
    for stem in seizure_stems:
        paths.extend(
            [
...
def _read_lzu_seizure(
    ...
    return SeizureRecord(
        ...
        seizure_offset_sec=float(offset) if offset is not None else None,
        labels=labels,
