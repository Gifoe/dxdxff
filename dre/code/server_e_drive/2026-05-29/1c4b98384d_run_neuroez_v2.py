...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--feature_view",
        type=str,
        default="self_comparison",
        choices=["self_comparison", "interictal_comparison", "dual_state_comparison", "absolute", "raw", "none"],
    )
    ...
    parser.add_argument("--interictal_baseline_mode", type=str, default="robust", choices=["robust"])
    parser.add_argument("--interictal_min_windows", type=int, default=10)
    parser.add_argument("--include_inter_abs", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--include_inter_delta", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--include_inter_zdelta", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--include_inter_ratio", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--include_inter_percentile", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--include_pre_comparison", type=_str_to_bool, nargs="?", const=True, default=False)
    parser.add_argument("--include_patient_relative", type=_str_to_bool, nargs="?", const=True, default=False)
    parser.add_argument("--include_patient_relative_z", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--include_patient_relative_rank", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--interictal_missing_indicators", type=_str_to_bool, nargs="?", const=True, default=True)

    parser.add_argument(
        "--adjacency_view",
        type=str,
        default="mixed_abs_delta",
        choices=["mixed_abs_delta", "mixed_interictal_delta", "mixed_dual_delta", "absolute", "abs", "raw", "none"],
    )
    ...
    parser.add_argument("--interictal_adjacency_alpha", type=float, default=0.5)
    parser.add_argument("--dual_adj_abs_weight", type=float, default=0.34)
    parser.add_argument("--dual_adj_inter_weight", type=float, default=0.33)
    parser.add_argument("--dual_adj_pre_weight", type=float, default=0.33)
    ...
    parser.add_argument("--use_physics_prior", type=_str_to_bool, nargs="?", const=True, default=False)
    parser.add_argument("--use_physics_features", type=_str_to_bool, nargs="?", const=True, default=False)
    parser.add_argument("--physics_feature_groups", type=str, default="ei,oscillation,causal,source_sink,hwc")
    parser.add_argument("--physics_normalization", type=str, default="interictal", choices=["interictal", "pre", "none"])
    parser.add_argument("--physics_missing_indicators", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--use_physics_feature_concat", type=_str_to_bool, nargs="?", const=True, default=False)
    parser.add_argument("--physics_logit_bias_beta", type=float, default=0.15)
    parser.add_argument("--physics_fusion_weight", type=float, default=0.25)
    parser.add_argument("--use_physics_soft_labels", type=_str_to_bool, nargs="?", const=True, default=True)
    parser.add_argument("--physics_soft_label_alpha", type=float, default=0.35)
    parser.add_argument("--physics_soft_label_gamma", type=float, default=0.50)
    parser.add_argument("--physics_rank_loss_weight", type=float, default=0.02)
    parser.add_argument("--physics_budget_loss_weight", type=float, default=0.02)
    parser.add_argument("--physics_budget_extra_fraction", type=float, default=0.15)
    parser.add_argument("--physics_budget_extra_min", type=float, default=1.0)
    parser.add_argument("--physics_budget_extra_max", type=float, default=5.0)

    parser.add_argument("--num_workers", type=int, default=0)
    parser.add_argument("--drop_high_ez_fraction_lzu", type=_str_to_bool, nargs="?", const=True, default=False)
    parser.add_argument("--lzu_max_ez_fraction", type=float, default=0.45)
