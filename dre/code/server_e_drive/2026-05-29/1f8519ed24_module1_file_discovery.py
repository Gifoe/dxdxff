...
def load_participants_table(participants_path: str) -> pd.DataFrame:
    participants_file = Path(participants_path)
    ...
    if "participant_id" in df.columns:
        ...
        df["participant_id"] = df["participant_id"].apply(
            lambda value: f"sub-{value}" if not value.startswith("sub-") else value
        )

    if "outcome" in df.columns:
        df["outcome"] = df["outcome"].astype(str).str.upper().str.strip()

        def _map_outcome(value: str) -> float:
            if value == "S":
                return 1.0
            if value == "F":
                return 0.0
        ...
        df["outcome_binary"] = df["outcome"].apply(_map_outcome)
    else:
        df["outcome"] = "UNKNOWN"
...
def _parse_subject_filter(subject_filter: str | None) -> list[str] | None:
    if not subject_filter:
        return None

    subject_ids = []
    for raw_value in re.split(r"[,;\s]+", str(subject_filter).strip()):
...
def discover_all_bids_files(
    ...
    subject_ids = _parse_subject_filter(subject_filter)
    if subject_ids:
        subject_dirs = [root_path / subject_id for subject_id in subject_ids]
    else:
        subject_dirs = [d for d in root_path.iterdir() if d.is_dir() and d.name.startswith("sub-")]
...
__all__ = ["discover_all_bids_files", "load_participants_table"]
