...
def _physics_soft_nez_bce_loss(
    logits: torch.Tensor,
    labels_nez: torch.Tensor,
    labels_ez: torch.Tensor,
    phys_risk: torch.Tensor,
    mask: torch.Tensor,
    *,
    ez_negative_weight: torch.Tensor,
    alpha: float = 0.35,
    ...
    valid = mask & (labels_nez >= 0.0)
    if not torch.any(valid):
        return logits.sum() * 0.0
    risk = phys_risk.to(logits.device).clamp(0.0, 1.0)
    target = labels_nez.clone()
    weights = torch.ones_like(labels_nez)

    known_ez = (labels_ez > 0.5) & valid
    clinical_nez = (labels_nez > 0.5) & valid
    target[known_ez] = 0.0
    weights[known_ez] = ez_negative_weight.to(logits.device)
    target[clinical_nez] = 1.0 - float(alpha) * risk[clinical_nez]
    weights[clinical_nez] = (1.0 - float(gamma) * risk[clinical_nez]).clamp_min(0.10)

    losses = F.binary_cross_entropy_with_logits(logits[valid], target[valid], reduction="none")
...
def _physics_ez_rank_loss(score_ez: torch.Tensor, phys_risk: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    losses: List[torch.Tensor] = []
    risk = phys_risk.to(score_ez.device).clamp(0.0, 1.0)
    for patient_idx in range(score_ez.shape[0]):
        valid = mask[patient_idx]
        if int(valid.sum().item()) < 2:
            continue
        s = score_ez[patient_idx][valid]
        r = risk[patient_idx][valid]
        delta_r = r[:, None] - r[None, :]
        pair_mask = delta_r.abs() > 0.05
        if not torch.any(pair_mask):
            continue
        direction = torch.sign(delta_r[pair_mask])
        delta_s = (s[:, None] - s[None, :])[pair_mask]
        weight = delta_r.abs()[pair_mask].detach()
        losses.append((F.softplus(-direction * delta_s) * weight).sum() / weight.sum().clamp_min(1e-6))
    if not losses:
        return score_ez.sum() * 0.0
    return torch.stack(losses).mean()


def _physics_ez_budget_loss(outputs: Dict[str, torch.Tensor], batch: Dict[str, Any], args: Any) -> torch.Tensor:
    score_ez = outputs["score_ez"]
    mask = batch["channel_mask"]
    labels_ez = batch["labels_ez"]
    valid_count = mask.float().sum(dim=1).clamp_min(1.0)
    true_ez_count = (labels_ez.clamp_min(0.0) * mask.float()).sum(dim=1)
    extra = float(getattr(args, "physics_budget_extra_fraction", 0.15)) * valid_count
    extra = torch.clamp(
        extra,
        min=float(getattr(args, "physics_budget_extra_min", 1.0)),
        max=float(getattr(args, "physics_budget_extra_max", 5.0)),
    )
    max_ez_count = true_ez_count + extra
    pred_ez_mass = (score_ez * mask.float()).sum(dim=1)
    return F.relu(pred_ez_mass - max_ez_count).pow(2).mean()
...
class Exp_EZHybridLocalization:
    ...
    def _compute_loss(self, outputs: Dict[str, torch.Tensor], batch: Dict[str, Any], ez_negative_weight: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        labels = batch["labels"]
        mask = batch["channel_mask"]
        use_physics = bool(getattr(self.args, "use_physics_prior", False)) and "physics_risk" in batch
        if use_physics and bool(getattr(self.args, "use_physics_soft_labels", True)):
            bce = _physics_soft_nez_bce_loss(
                logits,
                batch["labels_nez"],
                batch["labels_ez"],
                batch["physics_risk"],
                ...
                alpha=float(getattr(self.args, "physics_soft_label_alpha", 0.35)),
                gamma=float(getattr(self.args, "physics_soft_label_gamma", 0.50)),
            )
        else:
        ...
        physics_rank = (
            _physics_ez_rank_loss(outputs["score_ez"], batch["physics_risk"], mask)
            if use_physics and float(getattr(self.args, "physics_rank_loss_weight", 0.0)) > 0.0
            else logits.sum() * 0.0
        )
        physics_budget = (
            _physics_ez_budget_loss(outputs, batch, self.args)
            if use_physics and float(getattr(self.args, "physics_budget_loss_weight", 0.0)) > 0.0
            else logits.sum() * 0.0
        )
        ...
        total = (
            ...
            + float(getattr(self.args, "physics_rank_loss_weight", 0.0)) * physics_rank
            + float(getattr(self.args, "physics_budget_loss_weight", 0.0)) * physics_budget
        )
        parts = {
            ...
            "physics_rank": float(physics_rank.detach().cpu()),
            "physics_budget": float(physics_budget.detach().cpu()),
    ...
    def _train_one_epoch(
        ...
        physics_rank_losses: List[float] = []
        physics_budget_losses: List[float] = []
        for batch in loader:
            ...
            physics_rank_losses.append(parts["physics_rank"])
            physics_budget_losses.append(parts["physics_budget"])
        return {
            ...
            "physics_rank": float(np.mean(physics_rank_losses)) if physics_rank_losses else 0.0,
            "physics_budget": float(np.mean(physics_budget_losses)) if physics_budget_losses else 0.0,
        }
        ...
        *,
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                physics_risk_np = physics_risk.cpu().numpy() if torch.is_tensor(physics_risk) else None
                    ...
                    records.append(
                        {
                            ...
                            "physics_risk_ez": physics_risk_np[idx, :c].astype(np.float32) if physics_risk_np is not None else np.zeros((c,), dtype=np.float32),
    ...
    def _save_outputs(self, records: Sequence[Dict[str, Any]], *, fold_idx: int, split_name: str) -> None:
        ...
        for record in records:
            ...
            score_nez = np.asarray(record.get("score_nez", record["scores"]), dtype=np.float32)
            score_ez = np.asarray(record.get("score_ez", 1.0 - score_nez), dtype=np.float32)
            physics_risk = np.asarray(record.get("physics_risk_ez", np.zeros_like(score_nez)), dtype=np.float32)
                ...
                channel_rows.append(
                    {
                        ...
                        "physics_risk_ez": float(physics_risk[channel_idx]),
