...
def test_primary_cohort_can_encode_failure_as_the_positive_class() -> None:
    outcome = pd.DataFrame(
        [
            {"subject_id": "s1", "center": "c1", "outcome_group": "success", "outcome_label": 1},
            {"subject_id": "s2", "center": "c1", "outcome_group": "success", "outcome_label": 1},
            {"subject_id": "f1", "center": "c2", "outcome_group": "failure", "outcome_label": 0},
        ]
    )
    cohort, audit = build_task2_primary_cohort(outcome, old90_subjects={"s1", "s2"}, positive_class="failure")
    assert dict(zip(cohort["subject_id"], cohort["outcome_label"])) == {"s1": 0, "s2": 0, "f1": 1}
    assert audit["positive_class"] == "failure"
