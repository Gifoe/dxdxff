...
class PatientChannelClassifier(nn.Module):
    ...
    def forward(self, patient_channel_embedding: torch.Tensor, channel_mask: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        if self.use_a9v8_lcbo:
            ...
            score_positive = score_eval
            if self.positive_label == "nez":
                score_nez = score_positive
                score_ez = (1.0 - score_positive).masked_fill(~channel_mask, 0.0)
                semantics = "nez_probability"
            else:
                score_ez = score_positive
                score_nez = (1.0 - score_positive).masked_fill(~channel_mask, 0.0)
                semantics = "ez_probability"
            return {
                "logits": logits_eval,
                "scores": score_eval,
                "score_nez": score_nez,
