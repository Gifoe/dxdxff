...
def load_lcbo_fold_targets(
    target_dir: str | Path,
    *,
    fold_id: int,
    target_semantics: str = "ez",
    ...
    df = pd.read_csv(path)
    required = {"fold_id", "channel_name", "label_ez", "pseudo_core_q"}
    if str(teacher_mode).lower() in {"legacy_global_oof", "nested_outer_oof"}:
        required |= {"a9v3_oof_score"}
    if str(target_semantics).lower() == "nez":
        required |= {"label_nez"}
        if str(teacher_mode).lower() in {"legacy_global_oof", "nested_outer_oof"}:
            required |= {"teacher_nez_score"}
...
def build_lcbo_target_lookup(
    ...
    *,
    ...
    for _, row in targets.iterrows():
        ...
        mode_value = teacher_mode if teacher_mode is not None else row.get("teacher_mode", "legacy_global_oof")
        mode = str(mode_value).lower()
        availability_value = row.get("teacher_score_available", mode != "physiology_only")
        teacher_available = bool(mode != "physiology_only") if pd.isna(availability_value) else bool(availability_value)
        teacher_ez = pd.to_numeric(pd.Series([row.get("a9v3_oof_score", np.nan)]), errors="coerce").iloc[0]
        teacher_nez = pd.to_numeric(pd.Series([row.get("teacher_nez_score", np.nan)]), errors="coerce").iloc[0]
        teacher_value = teacher_nez if semantics == "nez" else teacher_ez
        payload = {
            "label_ez": float(row["label_ez"]),
            "pseudo_core_q": float(row["pseudo_core_q"]),
            "a9v3_oof_score": float(teacher_ez) if pd.notna(teacher_ez) else float("nan"),
            "label_positive": float(row["label_nez"] if semantics == "nez" else row["label_ez"]),
            "teacher_score": float(teacher_value) if pd.notna(teacher_value) else float("nan"),
            "teacher_score_available": teacher_available and pd.notna(teacher_value),
            "teacher_mode": mode,
...
def _lcbo_target_tensors(
    ...
    semantics = str(positive_label or (next(iter(lookup.values())).get("target_semantics", "ez") if lookup else "ez")).lower()
    labels_positive = 1.0 - labels_ez if semantics == "nez" else labels_ez
    core_target = torch.zeros_like(labels_positive)
    teacher_score = torch.zeros_like(labels_positive)
    ...
    for pi, sid in enumerate(subject_ids):
        ...
        for ci, channel in enumerate(canonical_channels[pi]):
            ...
            teacher_score[pi, ci] = float(item.get("teacher_score", item.get("a9v3_oof_score", 0.0)))
            target_mask[pi, ci] = True
    return labels_positive, core_target, teacher_score, target_mask


def _lcbo_positive_labels(batch: Dict[str, Any], args: Any, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    """Return the labels used by the model's declared positive-label convention."""
    positive_label = str(getattr(args, "positive_label", "ez")).lower()
    key = "labels_nez" if positive_label == "nez" else "labels_ez"
    if key not in batch:
        raise KeyError(f"LCBO requires {key} for positive_label={positive_label!r}.")
    return batch[key].to(device=device, dtype=dtype)
...
def _lcbo_patient_balanced_bce(logits: torch.Tensor, labels_positive: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    losses: List[torch.Tensor] = []
    for patient_idx in range(logits.shape[0]):
        valid = mask[patient_idx] & (labels_positive[patient_idx] >= 0.0)
        if not torch.any(valid):
            continue
        raw = F.binary_cross_entropy_with_logits(logits[patient_idx][valid], labels_positive[patient_idx][valid], reduction="none")
        pos = labels_positive[patient_idx][valid] > 0.5
        neg = ~pos
...
def _lcbo_asymmetric_clean_nez_bce(
    logits: torch.Tensor,
    labels_nez: torch.Tensor,
    mask: torch.Tensor,
    *,
    clinical_ez_scale: float,
) -> torch.Tensor:
    losses: List[torch.Tensor] = []
    scale = max(float(clinical_ez_scale), 0.0)
    for patient_idx in range(logits.shape[0]):
        valid = mask[patient_idx] & (labels_nez[patient_idx] >= 0.0)
        pos = valid & (labels_nez[patient_idx] > 0.5)
        neg = valid & (labels_nez[patient_idx] <= 0.5)
        components: List[torch.Tensor] = []
        weights: List[float] = []
        if torch.any(pos):
            components.append(F.binary_cross_entropy_with_logits(logits[patient_idx][pos], torch.ones_like(logits[patient_idx][pos])))
            weights.append(1.0)
        if torch.any(neg) and scale > 0.0:
            components.append(F.binary_cross_entropy_with_logits(logits[patient_idx][neg], torch.zeros_like(logits[patient_idx][neg])))
            weights.append(scale)
        if components:
            losses.append(sum(weight * value for weight, value in zip(weights, components)) / max(sum(weights), 1e-8))
    return torch.stack(losses).mean() if losses else logits.sum() * 0.0


def _weak_ez_mil_loss(
    score_ez: torch.Tensor,
    labels_ez: torch.Tensor,
    channel_mask: torch.Tensor,
    *,
    margin: float,
    ez_core_fraction: float,
    hard_nez_fraction: float,
) -> torch.Tensor:
    losses: List[torch.Tensor] = []
    for patient_idx in range(score_ez.shape[0]):
        valid = channel_mask[patient_idx]
        ez = score_ez[patient_idx][valid & (labels_ez[patient_idx] > 0.5)]
        nez = score_ez[patient_idx][valid & (labels_ez[patient_idx] <= 0.5)]
        if ez.numel() == 0 or nez.numel() == 0:
            continue
        k_ez = max(1, int(np.ceil(ez.numel() * float(ez_core_fraction))))
        k_nez = max(1, int(np.ceil(nez.numel() * float(hard_nez_fraction))))
        top_ez = ez.topk(k_ez, largest=True).values.mean()
        hard_nez = nez.topk(k_nez, largest=True).values.mean()
        losses.append(F.softplus(float(margin) + hard_nez - top_ez))
    return torch.stack(losses).mean() if losses else score_ez.sum() * 0.0


def _lcbo_core_rank_loss(
    logits_core: torch.Tensor,
    labels_positive: torch.Tensor,
    ...
    *,
    ...
    for patient_idx in range(logits_core.shape[0]):
        valid = channel_mask[patient_idx]
        core_positive = valid & target_mask[patient_idx] & (core_target[patient_idx] > 0.0)
        clinical_negative = valid & (labels_positive[patient_idx] <= 0.5)
        if not torch.any(core_positive) or not torch.any(clinical_negative):
            continue
        pos_mass = core_target[patient_idx][core_positive].sum()
        if float(pos_mass.detach().cpu()) < float(min_core_mass):
            continue
        neg_logits = logits_core[patient_idx][clinical_negative]
        ...
        pair = F.softplus(
            float(margin) + neg_logits.view(1, -1) - logits_core[patient_idx][core_positive].view(-1, 1)
        )
        per_positive = pair.mean(dim=1)
        weights = core_target[patient_idx][core_positive].clamp_min(0.0)
        losses.append((per_positive * weights).sum() / weights.sum().clamp_min(1e-6))
    if not losses:
...
def apply_teacher_anchor_eval_scores(
    ...
    if not bool(getattr(args, "use_teacher_anchor_eval", False)):
        return outputs
    if str(getattr(args, "teacher_mode", "physiology_only")).lower() == "physiology_only":
        raise RuntimeError("teacher anchor evaluation is forbidden in teacher_mode=physiology_only.")
    ...
    channel_mask = batch["channel_mask"].to(logits_eval.device).bool()
    positive_label = str(getattr(args, "positive_label", "ez")).lower()
    if positive_label == "nez":
        outputs["score_nez"] = outputs["score_eval"].masked_fill(~channel_mask, 0.0)
        outputs["score_ez"] = (1.0 - outputs["score_eval"]).masked_fill(~channel_mask, 0.0)
    else:
        outputs["score_ez"] = outputs["score_eval"].masked_fill(~channel_mask, 0.0)
        outputs["score_nez"] = (1.0 - outputs["score_eval"]).masked_fill(~channel_mask, 0.0)
...
def compute_a9v8_lcbo_loss(
    ...
    labels_positive, core_target, teacher_score, target_mask = _lcbo_target_tensors(
        batch,
        target_lookup,
        logits_broad.device,
        logits_broad.dtype,
        positive_label=str(getattr(args, "positive_label", "ez")),
    )
    target_stats = compute_lcbo_target_match_stats(batch, target_lookup)
    labels_ez = batch["labels_ez"].to(device=logits_broad.device, dtype=logits_broad.dtype)
    lcbo_loss_mode = str(getattr(args, "lcbo_loss_mode", "")).lower()
    if not lcbo_loss_mode:
        experiment_mode = str(getattr(args, "experiment_mode", "EZ_S5_8_CONTROL")).upper()
        lcbo_loss_mode = "asymmetric_clean_nez_lcbo" if experiment_mode == "NEZ_S5_8_ASYMMETRIC" else "symmetric_lcbo"
    if lcbo_loss_mode == "asymmetric_clean_nez_lcbo":
        broad_bce = _lcbo_asymmetric_clean_nez_bce(
            logits_broad,
            labels_positive,
            channel_mask,
            clinical_ez_scale=float(getattr(args, "clinical_ez_bce_scale", 0.25)),
        )
    else:
        broad_bce = _lcbo_patient_balanced_bce(logits_broad, labels_positive, channel_mask)
    ...
    teacher_mode = str(getattr(args, "teacher_mode", "physiology_only")).lower()
    core_distill = (
        _lcbo_core_distill_loss(logits_core, teacher_score, channel_mask, target_mask)
        if teacher_mode != "physiology_only" and float(getattr(args, "lambda_core_distill", 0.0)) > 0.0
        else logits_core.sum() * 0.0
    )
    weak_ez = (
        _weak_ez_mil_loss(
            outputs["score_ez"], labels_ez, channel_mask,
            margin=float(getattr(args, "weak_ez_margin", 0.05)),
            ez_core_fraction=float(getattr(args, "weak_ez_core_fraction", 0.20)),
            hard_nez_fraction=float(getattr(args, "weak_ez_hard_nez_fraction", 0.20)),
        )
        if lcbo_loss_mode == "asymmetric_clean_nez_lcbo" else logits_broad.sum() * 0.0
    )
    loss = (
        broad_bce
        ...
        + float(getattr(args, "lambda_weak_ez_mil", 0.10)) * weak_ez
    )
    ...
    parts = {
        ...
        "mean_weak_ez_mil_loss": float(weak_ez.detach().cpu()),
...
def _summarize_prediction_records(
    ...
    *,
    ...
    threshold_source: str = "validation_only",
    ...
    for record in records:
        ...
        record_threshold = float(record.get("validation_selected_threshold", 0.5)) if decision_threshold is None else float(decision_threshold)
        pred_nez_mask = (score_nez >= record_threshold) & valid_mask
        ...
        enriched_record["validation_selected_threshold"] = record_threshold
        enriched_record["threshold_source"] = str(threshold_source)
        enriched_record["predicted_ez_fraction"] = float(pred_ez_mask.sum() / max(int(valid_mask.sum()), 1))
        enriched_record["true_ez_fraction"] = float(y_ez.mean())
        enriched_record["predicted_nez_fraction"] = float(pred_nez_mask.sum() / max(int(valid_mask.sum()), 1))
        enriched_record["true_nez_fraction"] = float(y_nez.mean())
    ...
    summary = {
        ...
        "validation_selected_threshold": float(decision_threshold) if decision_threshold is not None else float("nan"),
    ...
    if pooled_y_nez:
        ...
        pooled_precision, pooled_recall, pooled_f1, _ = precision_recall_fscore_support(
            y_nez_all, pred_nez_all, labels=[1, 0], zero_division=0
        )
        summary.update({
            "pooled_nez_precision": float(pooled_precision[0]),
            "pooled_nez_recall": float(pooled_recall[0]),
            "pooled_nez_f1": float(pooled_f1[0]),
            "pooled_ez_precision": float(pooled_precision[1]),
            "pooled_ez_recall": float(pooled_recall[1]),
            "pooled_ez_f1": float(pooled_f1[1]),
        })
    else:
        summary.update(
            {
...
def select_best_decision_rule(records: Sequence[Dict[str, Any]], args: Any | None = None):
    ...
    for threshold in thresholds:
        ...
        for record in records:
            ...
            if not np.any(valid):
                continue
            y_nez = np.asarray(record.get("labels_nez", record["labels"]), dtype=np.float32)[valid].astype(int)
            score_nez = np.asarray(record.get("score_nez", record["scores"]), dtype=np.float32)[valid]
            patient_scores.append(float(f1_score(y_nez, score_nez >= threshold, average="macro", zero_division=0)))
        score = _safe_mean(patient_scores)
    ...
    summary, enriched = _summarize_prediction_records(
        records, decision_threshold=best_threshold, threshold_source="validation_only"
    )
    return {
        "strategy": "validation_only_threshold",
        "primary_hard_label_rule": "validation_only_threshold",
...
def compute_lcbo_config_hash(args: Any) -> str:
    keys = (
        "random_seed", "positive_label", "experiment_mode", "teacher_mode", "lcbo_loss_mode",
        "window_cache_path", "latent_core_target_dir", "model_dim", "num_heads", "dropout",
        "b0_feature_groups", "b0_feature_parts", "use_physics_dynamics", "physics_feature_parts",
        "physics_state_features", "temporal_pooling", "record_pooling", "eval_score_fusion_gamma",
        "lambda_core_rank", "lambda_soft_mrr", "lambda_subset", "lambda_core_distill",
        "lambda_weak_ez_mil", "clinical_ez_bce_scale", "core_rank_margin", "soft_mrr_tau",
        "subset_eps", "lcbo_hard_neg_top_frac", "lcbo_min_core_mass", "learning_rate",
...
def load_valid_lcbo_fold_resume(
    marker_path: str | Path,
    *,
    expected_config_hash: str,
    expected_prediction_path: str | Path,
) -> dict[str, Any] | None:
    """Return a completed fold marker only when every resumability invariant holds."""
    marker_path = Path(marker_path)
    prediction_path = Path(expected_prediction_path)
    if not marker_path.exists() or not prediction_path.exists():
        return None
    try:
        completed = json.loads(marker_path.read_text(encoding="utf-8"))
    except (OSError, ValueError, TypeError, json.JSONDecodeError):
        return None
    checkpoint_value = str(completed.get("checkpoint_path", "")).strip()
    if not checkpoint_value:
        return None
    checkpoint_path = Path(checkpoint_value)
    if (
        completed.get("status") != "complete"
        or completed.get("config_hash") != expected_config_hash
...
def _records_from_saved_test_csv(path: Path, *, threshold: float) -> list[Dict[str, Any]]:
    frame = pd.read_csv(path)
    required = {"subject_id", "channel_name", "true_ez", "true_nez", "score_ez", "score_nez"}
    missing = sorted(required - set(frame.columns))
    if missing:
        raise RuntimeError(f"Cannot resume from {path}; missing columns: {missing}")
        ...
        records.append(
            {
                ...
                "center_id": int(group["center_id"].iloc[0]) if "center_id" in group.columns else 4,
                "canonical_channels": group["channel_name"].astype(str).tolist(),
                "labels": group["true_ez"].to_numpy(dtype=np.float32),
                "labels_ez": group["true_ez"].to_numpy(dtype=np.float32),
                "labels_nez": group["true_nez"].to_numpy(dtype=np.float32),
                "score_ez": group["score_ez"].to_numpy(dtype=np.float32),
                "score_nez": group["score_nez"].to_numpy(dtype=np.float32),
                "scores": group["score_nez"].to_numpy(dtype=np.float32),
...
class Exp_EZHybridLocalization:
    ...
    def __init__(self, args: Any) -> None:
        self.args = args
        self.config_hash = compute_lcbo_config_hash(args)
        ...
        if self.use_a9v8_lcbo and str(getattr(args, "positive_label", "nez")).lower() not in {"ez", "nez"}:
            raise ValueError("A9v8 LCBO requires --positive_label ez or nez.")
    ...
    def _load_lcbo_targets_for_fold(self, fold_idx: int) -> None:
        ...
        semantics = str(getattr(self.args, "target_semantics", getattr(self.args, "positive_label", "ez"))).lower()
        teacher_mode = str(getattr(self.args, "teacher_mode", "physiology_only")).lower()
        target_df = load_lcbo_fold_targets(
            target_dir,
            fold_id=int(fold_idx),
            target_semantics=semantics,
            teacher_mode=teacher_mode,
        )
        self.current_lcbo_target_lookup = build_lcbo_target_lookup(
            target_df,
            target_semantics=semantics,
            teacher_mode=teacher_mode,
        )
        ...
        self.current_lcbo_eval_lookup = (
            build_lcbo_target_lookup(
                pd.read_csv(all_oof_path), target_semantics=semantics, teacher_mode=teacher_mode
            ) if all_oof_path.exists() else dict(self.current_lcbo_target_lookup)
        )
        self._log(
    ...
    def _compute_loss(
        ...
        *,
        ...
        if use_lcbo:
            ...
            broad_bce = outputs["logits_broad"].sum() * 0.0 + float(diag_parts["mean_broad_bce_loss"])
            parts = {
    ...
    def _evaluate(
        ...
        *,
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if bool(getattr(self, "use_a9v8_lcbo", False)) and getattr(self, "current_lcbo_eval_lookup", None):
                    ...
                    teacher_key = "teacher_nez_score" if str(getattr(self.args, "target_semantics", "ez")).lower() == "nez" else "a9v3_oof_score"
                    optional_channel_arrays[teacher_key] = teacher_np
        ...
        if str(split_name).lower() == "val":
            _, summary, enriched = select_best_decision_rule(records, self.args)
        else:
            summary, enriched = _summarize_prediction_records(
                records,
                decision_threshold=float(getattr(self, "current_decision_threshold", 0.5)),
        ...
        summary["strategy"] = "validation_only_threshold"
        ...
        for record in records:
            ...
            optional_channel_arrays = {
                ...
                for key in (
                    ...
                    "logits_base",
                    "pseudo_core_q", "a9v3_oof_score", "teacher_nez_score",
                    "negative_anchor_distance", "graph_delta_logits",
                )
                if key in record
            }
            ...
            for channel_idx, channel_name in enumerate(record["canonical_channels"]):
                channel_dict = {
                        ...
                        "score_nez": float(score_nez[channel_idx]),
                        "score_ez": float(score_ez[channel_idx]),
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        fold_ledger_rows: List[Dict[str, Any]] = []
        ...
        center_by_subject: Dict[str, str] = {}
        for record in self.run_records:
            center_by_subject[str(record.get("subject_id"))] = str(record.get("source_center", record.get("center", "unknown")))
        output_dir = Path(getattr(self.args, "output_dir", "outputs"))
        output_dir.mkdir(parents=True, exist_ok=True)
        pd.DataFrame([
            {"subject_id": subject_id, "center": center_by_subject.get(str(subject_id), "unknown")}
            for subject_id in sorted(map(str, self.patient_index))
        ]).to_csv(output_dir / "subject_manifest.csv", index=False)
        ...
        self._log(
            ...
            f"loss={str(getattr(self.args, 'loss_mode', 'masked_bce'))} | selection=validation_only_threshold"
        )

        for split in outer_splits:
            fold_idx = int(split["fold_idx"])
            fold_dir = output_dir / f"fold_{fold_idx}"
            fold_complete_path = fold_dir / "fold_complete.json"
            if bool(getattr(self.args, "skip_existing_folds", False)):
                test_path = output_dir / f"test_channel_predictions_neuroez_v2_fold_{fold_idx}.csv"
                completed = load_valid_lcbo_fold_resume(
                    fold_complete_path,
                    expected_config_hash=self.config_hash,
                    expected_prediction_path=test_path,
                )
                if completed is not None:
                    threshold = float(completed.get("validation_selected_threshold", 0.5))
                    resumed = _records_from_saved_test_csv(test_path, threshold=threshold)
                    all_test_records.extend(resumed)
                    stored_summary = completed.get("test_summary", {})
                    fold_summary_rows.append({"fold_id": fold_idx, "n_test_patients": len(resumed), **stored_summary})
                    threshold_audit_rows.append(
                        {
                            "fold_idx": fold_idx,
                            "validation_selected_threshold": threshold,
                            "threshold_source": "validation_only",
                            "selection_metric": "patient_macro_f1",
                            "test_labels_used_for_selection": False,
                            "resumed": True,
                        }
                    )
                    self._log(f"Fold {fold_idx}: resumed completed fold with matching config hash {self.config_hash[:12]}.")
                    continue
            ...
            for split_name, subjects in (("train", fit_subjects), ("val", val_subjects), ("test", test_subjects)):
                fold_ledger_rows.extend({"fold_idx": fold_idx, "split": split_name, "subject_id": str(subject)} for subject in subjects)
            ...
            normalizer_audit_rows.append({
                ...
                "n_fit_patients": len(fit_subjects),
                "n_val_patients": len(val_subjects),
                "n_test_patients": len(test_subjects),
                "val_in_normalizer": False,
                "test_in_normalizer": False,
            ...
            for epoch in range(1, epochs + 1):
                ...
                if improved:
                    ...
                    best_threshold = float(val_summary.get("validation_selected_threshold", 0.5))
                    epochs_without_improvement = 0
                    torch.save(
                        {
                            "epoch": epoch,
                            ...
                            "positive_label": str(getattr(self.args, "positive_label", "nez")),
                            "experiment_mode": str(getattr(self.args, "experiment_mode", "")),
                            "teacher_mode": str(getattr(self.args, "teacher_mode", "physiology_only")),
                            "lcbo_loss_mode": str(getattr(self.args, "lcbo_loss_mode", "")),
            ...
            self.current_decision_threshold = float(best_threshold)
            threshold_audit_rows.append({
                "fold_idx": int(fold_idx),
            ...
            fold_complete_path.parent.mkdir(parents=True, exist_ok=True)
            fold_complete_path.write_text(
                json.dumps(
                    {
                        "status": "complete",
                        "fold_id": int(fold_idx),
                        ...
                        "test_prediction_path": str(output_dir / f"test_channel_predictions_neuroez_v2_fold_{fold_idx}.csv"),
                        "validation_selected_threshold": float(best_threshold),
                        "n_test_patients": int(len(test_records)),
                        "test_summary": fold_summary_rows[-1],
                    },
                    indent=2,
                ),
                encoding="utf-8",
        ...
        if all_test_records:
            overall_summary, _ = _summarize_prediction_records(all_test_records, decision_threshold=None)
            output_dir = Path(getattr(self.args, "output_dir", "outputs"))
            output_dir.mkdir(parents=True, exist_ok=True)
            for key in (
                "positive_label",
            ...
            overall_summary.update(
                {
                    "teacher_mode": teacher_mode,
                    "teacher_used": teacher_mode != "physiology_only",
                    "teacher_outer_fold_leak_free": teacher_mode in {"physiology_only", "nested_outer_oof"},
                    "formal_primary_eligible": teacher_mode != "legacy_global_oof",
                    "lcbo_loss_mode": str(getattr(self.args, "lcbo_loss_mode", "")),
                    "primary_hard_label_rule": "validation_only_threshold",
                    "true_k_usage": "ranking_diagnostic_only",
                    "test_true_count_used_for_primary_labels": False,
                    "exact12_claimed": False,
                    "feature_mode": "S5_8_NO_HFO",
                }
            )
            pd.DataFrame([overall_summary]).to_csv(output_dir / "heldout_summary_neuroez_v3.csv", index=False)
            pd.DataFrame([overall_summary]).to_csv(output_dir / "heldout_summary_nez_s5_8.csv", index=False)
            pd.DataFrame(fold_summary_rows).to_csv(output_dir / "heldout_fold_summary_neuroez_v3.csv", index=False)
            pd.DataFrame(fold_summary_rows).to_csv(output_dir / "heldout_summary_by_fold.csv", index=False)
            pd.DataFrame(threshold_audit_rows).to_csv(output_dir / "threshold_audit_by_fold.csv", index=False)
            ...
            if self.use_a9v8_lcbo:
                ...
                pd.DataFrame(lcbo_train_loss_rows).to_csv(output_dir / "loss_components_by_epoch.csv", index=False)
            center_rows = []
            for center in sorted({str(record.get("center", "unknown")) for record in all_test_records}):
                center_records = [record for record in all_test_records if str(record.get("center", "unknown")) == center]
                center_summary, _ = _summarize_prediction_records(center_records, decision_threshold=None)
                center_rows.append({"center": center, **center_summary})
            pd.DataFrame(center_rows).to_csv(output_dir / "heldout_summary_by_center.csv", index=False)
            pd.Series(overall_summary).to_json(output_dir / "heldout_summary_neuroez_v3.json", indent=2)
            pd.Series(overall_summary).to_json(output_dir / "heldout_summary_nez_s5_8.json", indent=2)
            channel_paths = sorted(output_dir.glob("test_channel_predictions_neuroez_v2_fold_*.csv"))
            channel_frames = [pd.read_csv(path) for path in channel_paths]
            all_channels = pd.concat(channel_frames, ignore_index=True) if channel_frames else pd.DataFrame()
            if not all_channels.empty:
                all_channels.to_csv(output_dir / "oof_channel_predictions.csv", index=False)
            for path in channel_paths:
                fold_token = path.stem.rsplit("_", 1)[-1]
                pd.read_csv(path).to_csv(output_dir / f"test_channel_predictions_fold_{fold_token}.csv", index=False)
            for path in sorted(output_dir.glob("val_channel_predictions_neuroez_v2_fold_*.csv")):
                fold_token = path.stem.rsplit("_", 1)[-1]
                pd.read_csv(path).to_csv(output_dir / f"val_channel_predictions_fold_{fold_token}.csv", index=False)
            duplicate_rows = int(all_channels.duplicated(["subject_id", "channel_name"]).sum()) if not all_channels.empty else 0
            observed_subjects = len({str(record.get("subject_id")) for record in all_test_records})
            full_run = max_outer_folds <= 0
            teacher_outer_fold_leak_free = teacher_mode in {"physiology_only", "nested_outer_oof"}
            all_predictions_finite = bool(
                all_channels.drop(columns=["a9v3_oof_score", "teacher_nez_score"], errors="ignore")
                .select_dtypes(include=[np.number]).apply(np.isfinite).all().all()
            ) if not all_channels.empty else False
            protocol_failure_reasons: list[str] = []
            if teacher_mode == "legacy_global_oof":
                protocol_failure_reasons.append("Legacy global OOF teacher is not nested within the current outer fold.")
            if not full_run:
                protocol_failure_reasons.append("Smoke or partial outer-fold run is not a formal all90 result.")
            if observed_subjects != 90:
                protocol_failure_reasons.append(f"Expected 90 held-out patients, observed {observed_subjects}.")
            if len(outer_splits) != 5:
                protocol_failure_reasons.append(f"Expected 5 outer folds, observed {len(outer_splits)}.")
            if len(self.run_records) != 281:
                protocol_failure_reasons.append(f"Expected 281 runs, observed {len(self.run_records)}.")
            if duplicate_rows != 0:
                protocol_failure_reasons.append(f"Duplicate patient-channel rows: {duplicate_rows}.")
            if not all_predictions_finite:
                protocol_failure_reasons.append("One or more saved prediction values are non-finite.")
            if not teacher_outer_fold_leak_free:
                protocol_failure_reasons.append("Teacher scores are not outer-fold leak-free.")
            protocol_audit = {
                "expected_patient_count": 90,
                "observed_patient_count": observed_subjects,
                "expected_run_count": 281,
                "observed_run_count": len(self.run_records),
                "expected_outer_folds": 5,
                "observed_outer_folds": len(outer_splits),
                "patient_disjoint_folds": True,
                "complete_oof_coverage": observed_subjects == 90 if full_run else False,
                "smoke_partial_oof": not full_run,
                "train_only_normalization": True,
                "validation_only_checkpoint_selection": True,
                "validation_only_threshold_selection": True,
                "no_test_true_count_in_primary_labels": True,
                "positive_label": str(getattr(self.args, "positive_label", "nez")),
                "score_semantics": str(getattr(self.args, "score_semantics", "nez_probability")),
                "feature_mode": "S5_8_NO_HFO",
                "teacher_mode": teacher_mode,
                "teacher_used": teacher_mode != "physiology_only",
                "teacher_outer_fold_leak_free": teacher_outer_fold_leak_free,
                "formal_primary_eligible": teacher_mode != "legacy_global_oof" and teacher_outer_fold_leak_free,
                "formal_protocol_pass": len(protocol_failure_reasons) == 0,
                "formal_protocol_failure_reasons": protocol_failure_reasons,
                "formal_protocol_failure_reason": "; ".join(protocol_failure_reasons) if protocol_failure_reasons else None,
                "lcbo_loss_mode": str(getattr(self.args, "lcbo_loss_mode", "")),
                "primary_hard_label_rule": "validation_only_threshold",
                "true_k_usage": "ranking_diagnostic_only",
                "test_true_count_used_for_primary_labels": False,
                ...
                "duplicate_patient_channel_rows": duplicate_rows,
            }
            if full_run and (observed_subjects != 90 or len(outer_splits) != 5 or duplicate_rows != 0):
                raise RuntimeError(f"Formal protocol audit failed: {protocol_audit}")
        ...
        output_dir = Path(getattr(self.args, "output_dir", "outputs"))
        pd.DataFrame(fold_ledger_rows).to_csv(output_dir / "fold_ledger.csv", index=False)
        pd.DataFrame(normalizer_audit_rows).to_csv(output_dir / "normalizer_audit_by_fold.csv", index=False)
