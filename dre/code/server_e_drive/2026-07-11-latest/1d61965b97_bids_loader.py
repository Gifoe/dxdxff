...
from .bids_common import (
    ...
    participant_surgery_success_with_source,
    raw_norm_to_name,
...
from .edf import close_raw, finalize_raw_data, read_raw_edf
from .schemas import (
    DataInterfaceConfig,
    PatientRecord,
    SeizureRecord,
...
def load_bids_patient_records(
    ...
    edf_files, matched_dirs = discover_bids_edfs_for_participants(
        root=root,
        participants=success_participants if success_filter_applies else None,
...
def _read_bids_seizure(
    ...
    surgery_success, outcome_source = participant_surgery_success_with_source(participant_meta)
    outcome_group = "success" if surgery_success is True else "failure" if surgery_success is False else "unknown"
    engel_score = participant_meta.get("engel_score", participant_meta.get("engel"))
    # Legacy success-only filter: may default unknown outcomes to False.
    success_used_legacy_filter = (
        is_successful_participant(participant_meta) if dataset_name in {"hup", "multicenter"} else None
    )
        ...
        meta.update(
            {
                ...
                "outcome_raw": participant_meta.get("outcome"),
                "outcome": participant_meta.get("outcome"),
                "success_used": success_used_legacy_filter,
                "success_used_legacy_filter": success_used_legacy_filter,
    ...
    return SeizureRecord(
        ...
        seizure_onset_sec=float(event["onset"]),
        seizure_offset_sec=float(event["offset"]) if event.get("offset") is not None else None,
        labels=labels,
