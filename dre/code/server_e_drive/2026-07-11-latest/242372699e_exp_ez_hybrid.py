...
def _summarize_prediction_records(
    ...
    for record in records:
        ...
        enriched_record["threshold_source"] = str(record.get("threshold_source", "not_used")) if decision_rule in explicit_mask_rules else str(record.get("threshold_source", "fixed"))
    ...
    summary = {
        "classification_threshold": float("nan") if enriched and all(
            str(record.get("decision_rule", "")).lower() == "predicted_nez_cardinality_topk" for record in enriched
        ) else float(
            classification_threshold
            if classification_threshold is not None
    ...
    summary["decision_rule"] = (
        "predicted_nez_cardinality_topk"
        if enriched and all(str(record.get("decision_rule", "")).lower() == "predicted_nez_cardinality_topk" for record in enriched)
        else "fixed_probability_threshold"
    )
    summary["threshold_source"] = "not_used" if summary["decision_rule"] == "predicted_nez_cardinality_topk" else "fixed_or_validation"
...
class Exp_EZHybridLocalization:
    ...
    def __init__(self, args: Any) -> None:
        ...
        if (self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez) and bool(getattr(args, "use_causal_propagation_residual", False)):
        ...
        if bool(getattr(args, "enforce_fixed_all90_protocol", False)):
            ...
            output_dir = Path(getattr(args, "output_dir", "outputs"))
            output_dir.mkdir(parents=True, exist_ok=True)
            protocol_filename = "sensitivity80_protocol_audit.json" if (self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez) else "fixed_all90_protocol_audit.json"
    ...
    def _build_datasets(
        ...
        *,
        ...
        if self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez:
