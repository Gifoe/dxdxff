...
class TRACERawMIL(nn.Module):
 ...
 def forward(self,patients):
  ...
  for patient in patients:
   ...
   u=torch.stack([x[0] for x in seizures]);rho=torch.stack([x[1] for x in seizures]);sl=torch.stack([x[2] for x in seizures]);center=u.mean(0);dev=((u-center)**2).mean(1);beta=torch.softmax(self.reliability(u).squeeze(-1)-torch.nn.functional.softplus(self.theta_deviation)*dev,0);logit=self.head(torch.cat([(beta[:,None]*u).sum(0),u.amax(0)])).squeeze();outputs.append({'logit':logit,'rho':(beta*rho).sum(),'beta':beta,'seizure_logits':sl,'details':[x[3] for x in seizures]})
  return outputs
