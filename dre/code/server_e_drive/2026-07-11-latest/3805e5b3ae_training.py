...
def run_simple_baseline_oof(summary: pd.DataFrame, fold_ledger: pd.DataFrame, *, model_name: str, seed: int, inner_folds: int = 4, compact: bool = False) -> SimpleBaselineResult:
    ...
    for fold in sorted(frame["fold_idx"].unique()):
        ...
        calibrated = calibrator.predict(raw_logit)
        ...
        test["calibration_method"] = "platt_inner_oof"
        test["threshold_source"] = "inner_oof_patient_macro_f1"
...
def run_simple_baseline_loco(
    ...
    *,
    ...
    result = run_simple_baseline_oof(
        summary, ledger, model_name=model_name, seed=seed, inner_folds=inner_folds, compact=compact
    )
