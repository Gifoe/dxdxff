...
def _build_standard_patient_rows(channels: pd.DataFrame) -> list[dict[str, float | str | int]]:
    rows: list[dict[str, float | str | int]] = []
    for col in ("score_eval", "score_broad", "score_core", "a9v3_oof_score"):
        if col in channels.columns:
            _append_rows(
                rows,
                channels,
                scores=channels[col].astype(float).to_numpy(),
    ...
    for gamma in GAMMA_GRID:
        _append_rows(
            rows,
            channels,
            scores=_sigmoid(broad_logit + float(gamma) * core_logit),
...
def _build_teacher_anchor_patient_rows(channels: pd.DataFrame) -> list[dict[str, float | str | int]]:
    if "a9v3_oof_score" not in channels.columns:
        raise RuntimeError("Missing a9v3_oof_score. Re-run LCBO evaluation with pseudo-core diagnostics enabled.")
    rows: list[dict[str, float | str | int]] = []
    ...
    for _, idx in grouped.indices.items():
        ...
        z_teacher[idx_arr] = patient_zscore(channels.iloc[idx_arr]["a9v3_oof_score"].astype(float).to_numpy())
...
def rescore_run_dir(
    ...
    *,
    ...
    standard_rows = _build_standard_patient_rows(channels)
    teacher_rows = _build_teacher_anchor_patient_rows(channels)
    summary, by_center, by_fold, _ = _write_outputs(root, "lcbo_rescore", standard_rows)
    teacher_summary, teacher_by_center, teacher_by_fold, _ = _write_outputs(root, "lcbo_teacher_anchor_rescore", teacher_rows)
