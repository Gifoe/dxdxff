...
def build_feature_estimator(name: str, params: dict[str, Any], *, seed: int):
    ...
    if normalized == "random_forest":
        classifier = RandomForestClassifier(n_estimators=50 if params.pop("_compact", False) else 500, class_weight="balanced_subsample", random_state=seed, n_jobs=-1, **params)
    ...
    if normalized == "lightgbm":
        ...
        classifier = LGBMClassifier(objective="binary", n_estimators=500, random_state=seed, verbosity=-1, **params)
        return Pipeline([("imputer", SimpleImputer()), ("classifier", classifier)])
    raise ValueError(f"Unknown Task 1 feature model: {name}")
