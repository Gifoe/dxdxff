...
try:
    # Package import is used by inference-only consumers from the repository
    # root. It must resolve the exact P23 runtime modules used for training.
    ...
    # The historical training entrypoint runs with P23_TRN_NEZ_80 as sys.path
    # root, where neuroez_c is a top-level package and parent-relative imports
    # are unavailable.
    from graph_spectral_encoder import WindowGraphSpectralEncoder
    from patient_channel_ranker import PatientChannelClassifier
    from seizure_aggregator import CrossSeizureMILAggregator
...
from .cane_path_cp_heads import (
    CausalPropagationResidual,
    CleanNEZPrototypeAnchor as PathCleanNEZPrototypeAnchor,
    MultiSeizureNEZEvidenceResidual as PathMultiSeizureNEZEvidenceResidual,
)
...
from .p23_profiles import get_p23_profile
from .p23_temporal_contrast import P23TemporalContrastEncoder
from .p23_seizure_tail import P23CrossSeizureTailEvidence, tail_reliability
from .p23_selective_fusion import P23SelectiveFusion
from .p2_rtc_profiles import get_rtc_profile
from .p2_atc_profiles import get_atc_profile
from .p2_scope_v2 import P2ScopeV2BoundaryReranker, P2ScopeV2CardinalityHead
...
class NeuroEZCModel(nn.Module):
    ...
    def __init__(self, args: Any | None = None) -> None:
        ...
        self.use_cane_set_nez = bool(getattr(args, "use_cane_set_nez", False) if args is not None else False)
        self.use_cane_path_cp_nez = bool(getattr(args, "use_cane_path_cp_nez", False) if args is not None else False)
        self.use_p21_v3_asrr_nez = bool(getattr(args, "use_p21_v3_asrr_nez", False) if args is not None else False)
        self.p21_profile = get_p21_profile(getattr(args, "p21_profile", "R5_FULL")) if self.use_p21_v3_asrr_nez else None
        self.use_p2_rtc_shift = bool(getattr(args, "use_p2_rtc_shift", False) if args is not None else False)
        self.use_p2_atc = bool(getattr(args, "use_p2_atc", False) if args is not None else False)
        self.use_p2_scope_v2 = bool(getattr(args, "use_p2_scope_v2", False) if args is not None else False)
        self.use_p23_trn_nez = bool(getattr(args, "use_p23_trn_nez", False) if args is not None else False) or self.use_p2_rtc_shift or self.use_p2_atc or self.use_p2_scope_v2
        self.rtc_profile = get_rtc_profile(getattr(args, "p2_rtc_profile", "A0")) if self.use_p2_rtc_shift else None
        self.atc_profile = get_atc_profile(getattr(args, "p2_atc_profile", "A0")) if self.use_p2_atc else None
        # RTC profiles reuse the established P2 temporal/q10 backbone and
        # change only the explicitly declared tail, loss, selection and shift layers.
        p23_name = "P2_TEMPORAL_Q10" if (self.use_p2_rtc_shift or self.use_p2_atc or self.use_p2_scope_v2) else getattr(args, "p23_profile", "P5_FULL")
        self.p23_profile = get_p23_profile(p23_name) if self.use_p23_trn_nez else None
        self.use_causal_propagation_residual = bool(getattr(args, "use_causal_propagation_residual", False) if args is not None else False)
        ...
        if self.use_cane_set_nez:
            incompatible = {
                ...
                "use_hard_topk_loss": bool(getattr(args, "use_hard_topk_loss", False)),
                "use_ez_ranking_loss": bool(getattr(args, "use_ez_ranking_loss", False)),
            }
            invalid = [name for name, enabled in incompatible.items() if enabled]
            if invalid:
                raise ValueError(f"CANE-Set-NEZ configuration is incompatible with: {invalid}")
        if self.use_cane_path_cp_nez:
            incompatible = {
                "positive_label": self.positive_label != "nez",
                "use_cane_set_nez": self.use_cane_set_nez,
                "use_n6_dual_view_ema": self.use_n6_dual_view_ema,
                "use_a9v8_lcbo": self.use_a9v8_lcbo,
                "use_two_expert_router": self.use_two_expert_router,
                "use_feature_separated_two_expert": self.use_feature_separated_two_expert,
            ...
            invalid = [name for name, enabled in incompatible.items() if enabled]
            if invalid:
                raise ValueError(f"CANE-PATH-CP configuration is incompatible with: {invalid}")
        if self.use_p21_v3_asrr_nez:
            incompatible = {
                "positive_label": self.positive_label != "nez",
                "use_cane_path_cp_nez": self.use_cane_path_cp_nez,
                "use_cane_set_nez": self.use_cane_set_nez,
                "use_n6_dual_view_ema": self.use_n6_dual_view_ema,
                "use_a9v8_lcbo": self.use_a9v8_lcbo,
                "use_two_expert_router": self.use_two_expert_router,
            ...
            invalid = [name for name, enabled in incompatible.items() if enabled]
            if invalid:
                raise ValueError(f"P2.1 V3-ASRR configuration is incompatible with: {invalid}")
        ...
        if self.use_cane_set_nez:
            self.clean_nez_anchor = CleanNEZPrototypeAnchor(
                input_dim=agg_out_dim,
                projection_dim=int(getattr(args, "cane_prototype_dim", 16)),
                num_prototypes=int(getattr(args, "cane_num_nez_prototypes", 4)),
                temperature=float(getattr(args, "cane_prototype_temperature", 0.10)),
                max_residual=float(getattr(args, "cane_max_anchor_residual", 0.30)),
                prototype_similarity_margin=float(getattr(args, "cane_prototype_similarity_margin", 0.50)),
                initial_gate_logit=float(getattr(args, "cane_anchor_gate_init", -8.0)),
            )
            self.multi_seizure_evidence = MultiSeizureNEZEvidenceResidual(
                input_dim=self.model_dim,
                max_residual=float(getattr(args, "cane_max_seizure_residual", 0.25)),
            )
            self.cardinality_head = PatientNEZCardinalityHead(input_dim=agg_out_dim)
        if self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez or self.use_p23_trn_nez:
            self.clean_nez_anchor = PathCleanNEZPrototypeAnchor(
                input_dim=agg_out_dim,
                projection_dim=int(getattr(args, "cane_prototype_dim", 16)),
                num_prototypes=int(getattr(args, "cane_num_nez_prototypes", 4)),
                temperature=float(getattr(args, "cane_prototype_temperature", 0.10)),
                max_residual=float(getattr(args, "cane_max_anchor_residual", 0.20)),
                prototype_similarity_margin=float(getattr(args, "cane_prototype_similarity_margin", 0.50)),
                initial_gate_logit=float(getattr(args, "cane_anchor_gate_init", -8.0)),
            )
            self.multi_seizure_evidence = PathMultiSeizureNEZEvidenceResidual(
                input_dim=self.model_dim,
                max_residual=float(getattr(args, "cane_max_seizure_residual", 0.20)),
                p21_hidden_dim=int(getattr(args, "p21_seizure_hidden_dim", 12)),
                enable_p21_head=self.use_p21_v3_asrr_nez and not self.p21_profile.legacy_p2,
            )
            self.causal_propagation_residual = CausalPropagationResidual(
                max_residual=float(getattr(args, "cane_max_causal_residual", 0.15)),
                enable_p21_head=self.use_p21_v3_asrr_nez and not self.p21_profile.legacy_p2,
            )
        if self.use_p23_trn_nez:
            self.p23_temporal_contrast = P23TemporalContrastEncoder(
                self.model_dim,
                onset_start_sec=float(getattr(args, "p23_onset_start_sec", 0.0)),
                onset_end_sec=float(getattr(args, "p23_onset_end_sec", 10.0)),
                spread_end_sec=float(getattr(args, "p23_spread_end_sec", 30.0)),
                max_gate=float(getattr(args, "p23_temporal_max_gate", 0.20)),
                gate_init=float(getattr(args, "p23_temporal_gate_init", -3.0)),
            )
            self.p23_seizure_tail = P23CrossSeizureTailEvidence(
                self.model_dim, hidden_dim=int(getattr(args, "p23_seizure_hidden_dim", 12)),
                robust_tail=bool(
                    (self.rtc_profile and self.rtc_profile.robust_tail)
                    or (self.atc_profile and self.atc_profile.robust_tail)
                ),
                tau=float(getattr(args, "p2_atc_robust_tail_tau", 0.25) if self.atc_profile else getattr(args, "rtc_tail_tau", 0.25)),
                feature_mode=("atc7" if self.atc_profile and self.atc_profile.robust_tail else "auto"),
                quantile=float(getattr(args, "p23_seizure_quantile", 0.10)),
            )
            fusion_priors = [
            ...
            if self.p23_profile.causal:
                fusion_priors.append(float(getattr(args, "p23_causal_gate_prior", 0.02)))
            self.p23_fusion = P23SelectiveFusion(
                agg_out_dim, use_causal=self.p23_profile.causal,
                hidden_dim=int(getattr(args, "p23_gate_hidden_dim", 16)),
                max_total_correction=float(getattr(args, "p23_max_total_correction", 0.20)),
                priors=tuple(fusion_priors),
            )
            if self.use_p2_scope_v2:
                self.p2_scope_boundary = P2ScopeV2BoundaryReranker(agg_out_dim, hidden_dim=int(getattr(args, "scope_boundary_hidden_dim", 32)), max_residual=float(getattr(args, "scope_boundary_max_residual", .10)))
                self.p2_scope_cardinality = P2ScopeV2CardinalityHead(agg_out_dim)
                self.register_buffer("scope_fit_global_ez_fraction_prior", torch.tensor(.25))
            if self.p23_profile.causal:
                self.p23_causal_head = nn.Sequential(nn.LayerNorm(6), nn.Linear(6, 8), nn.GELU(), nn.Dropout(0.10), nn.Linear(8, 1), nn.Tanh())
                nn.init.zeros_(self.p23_causal_head[-2].weight)
                nn.init.zeros_(self.p23_causal_head[-2].bias)
        if self.use_p21_v3_asrr_nez and not self.p21_profile.legacy_p2:
            priors = (
                float(getattr(args, "p21_gate_prior_noop", 0.90)),
                float(getattr(args, "p21_gate_prior_anchor", 0.04)),
                float(getattr(args, "p21_gate_prior_seizure", 0.04)),
                float(getattr(args, "p21_gate_prior_causal", 0.02)),
            )
            self.p21_selective_fusion = SelectiveResidualFusion(
                embedding_dim=agg_out_dim,
                hidden_dim=int(getattr(args, "p21_gate_hidden_dim", 16)),
                max_total_correction=float(getattr(args, "p21_max_total_correction", 0.20)),
                priors=priors,
            )
    ...
    def forward(self, batch: dict[str, Any]) -> dict[str, Any]:
        if self.use_p2_scope_v2:
            return self._forward_p2_scope_v2(batch)
        if self.use_p23_trn_nez:
            if self.p23_profile.name == "P0_CURRENT_P2":
                return self._forward_cane_path_cp_nez(batch)
            return self._forward_p23_trn_nez(batch)
        if self.use_p21_v3_asrr_nez:
            return self._forward_p21_v3_asrr_nez(batch)
        if self.use_cane_path_cp_nez:
            return self._forward_cane_path_cp_nez(batch)
        if self.use_cane_set_nez:
            return self._forward_cane_set_nez(batch)
        if self.use_n6_dual_view_ema:
    ...
    def _forward_p2_scope_v2(self, batch: dict[str, Any]) -> dict[str, Any]:
        """SCOPE prediction excludes all legacy anchor/seizure/causal residuals."""
        output = self._forward_p23_trn_nez(batch)
        mask = output["channel_mask"]
        boundary = self.p2_scope_boundary(output["contextual_channel_embedding"], output["direct_nez_logit"], output["seizure_nez_probability_q10"], output["temporal_delta_norm"], output["delta_onset_norm"], output["delta_spread_norm"], output["valid_seizure_count"], mask)
        scope_logit = (output["direct_nez_logit"] + boundary["scope_boundary_delta"]).masked_fill(~mask, -1e9)
        score_nez = torch.sigmoid(scope_logit).masked_fill(~mask, 0.0)
        counts = self.p2_scope_cardinality(output["contextual_channel_embedding"], scope_logit, output["seizure_nez_probability_q10"], output["temporal_delta_norm"], output["delta_onset_norm"], output["delta_spread_norm"], batch["seizure_mask"], mask, float(self.scope_fit_global_ez_fraction_prior.item()))
        output.update(boundary); output.update(counts)
        output.update({"logits": scope_logit, "final_nez_logit": scope_logit, "final_nez_logits": scope_logit, "scope_nez_logit": scope_logit, "scores": score_nez, "score_nez": score_nez, "final_score_nez": score_nez, "score_ez": (1-score_nez).masked_fill(~mask, 0.0), "final_score_ez": (1-score_nez).masked_fill(~mask, 0.0), "scope_score_nez": score_nez, "scope_score_ez": (1-score_nez).masked_fill(~mask, 0.0), "scope_ez_score": (-scope_logit).masked_fill(~mask, -1e9), "decision_rule": "predicted_k_topk"})
        return output

    def _forward_p23_trn_nez(self, batch: dict[str, Any]) -> dict[str, Any]:
        """P23: one score-level bounded correction; labels and center are never inputs."""
        channel_mask = batch["channel_mask"].bool()
        seizure_channel_mask = batch["seizure_channel_mask"].bool()

        def require_finite(name: str, value: torch.Tensor, mask: torch.Tensor | None = None) -> None:
            ...
            if not bool(valid.all()):
                raise FloatingPointError(f"P23 non-finite tensor before fusion: {name}")
        if not self.use_physics_dynamics or "physics_features" not in batch:
            raise KeyError("P23 requires Step4B physics features")
        ...
        physics_gate = torch.sigmoid(self.physics_gate(torch.cat((h_b0, h_dyn), dim=-1)))
        fused = self.m1_norm(h_b0 + physics_gate * h_dyn)
        seizure_embedding, temporal_weights = self.temporal_encoder(
            fused, seizure_channel_mask, window_mask=batch.get("window_mask")
        )
        temporal = self.p23_temporal_contrast(
            fused, batch["window_centers"], batch["window_mask"], seizure_channel_mask
        )
        require_finite("temporal_delta", temporal["temporal_delta"], seizure_channel_mask)
        if self.p23_profile.temporal and not bool(batch.get("p23_disable_temporal", False)):
            seizure_embedding = seizure_embedding + temporal["temporal_delta"]
        temporal_valid = batch["seizure_mask"].bool().unsqueeze(-1) & seizure_channel_mask
        temporal_count = temporal_valid.sum(dim=1).clamp_min(1).to(fused.dtype)
        def patient_temporal(value: torch.Tensor) -> torch.Tensor:
            return (value * temporal_valid.to(value.dtype)).sum(dim=1) / temporal_count
        temporal_patient = {
            name: patient_temporal(temporal[name].to(fused.dtype))
            for name in ("temporal_gate", "delta_onset_norm", "delta_spread_norm", "delta_late_norm", "slope_norm", "temporal_bin_valid_fraction")
        }
        temporal_delta_norm = patient_temporal(temporal["temporal_delta"].norm(dim=-1))
        patient_embedding, seizure_weights = self.seizure_aggregator(
            seizure_embedding, batch["seizure_mask"], seizure_channel_mask
        )
        direct = self.channel_classifier(patient_embedding, channel_mask)
        direct_logits = direct["logits"]
        require_finite("direct_nez_logit", direct_logits, channel_mask)
        anchor = self.clean_nez_anchor(direct["contextual_channel_embedding"], channel_mask)
        tail = self.p23_seizure_tail(seizure_embedding, batch["seizure_mask"], seizure_channel_mask)
        if self.rtc_profile is not None and self.rtc_profile.tail_reliability:
            reliability = tail_reliability(tail["valid_seizure_count"], tail["seizure_agreement"])
            tail["tail_reliability"] = reliability.masked_fill(~channel_mask, 0.0)
            tail["tail_residual_suppressed"] = (tail_reliability <= 1e-6) & channel_mask
            tail["u_seizure"] = tail["u_seizure"] * tail["tail_reliability"]
        else:
            tail["tail_reliability"] = torch.ones_like(tail["u_seizure"]).masked_fill(~channel_mask, 0.0)
            tail["tail_residual_suppressed"] = torch.zeros_like(channel_mask)
        # Lite forbids a q10 residual when there is no cross-seizure evidence.
        if int(self.p23_profile.min_seizures_for_tail) > 1:
            tail_enabled = tail["valid_seizure_count"] >= int(self.p23_profile.min_seizures_for_tail)
            tail["u_seizure"] = tail["u_seizure"].masked_fill(~tail_enabled, 0.0)
        require_finite("anchor_evidence", anchor["u_anchor"], channel_mask)
        require_finite("seizure_tail_evidence", tail["u_seizure"], channel_mask)
        tail["seizure_nez_agreement"] = tail["seizure_agreement"]
        tail["seizure_residual"] = torch.zeros_like(direct_logits)
        zeros = torch.zeros_like(direct_logits)
        cp_features = batch.get("causal_propagation_features")
        if cp_features is None:
            cp_features = torch.zeros((*channel_mask.shape, 6), device=direct_logits.device, dtype=direct_logits.dtype)
        causal = {
            "u_causal": zeros, "raw_u_causal": zeros, "q_cp": zeros,
            "causal_feature_valid": torch.zeros_like(channel_mask), "causal_residual": zeros,
            "causal_residual_saturation_rate": direct_logits.sum() * 0.0,
        }
        if self.p23_profile.causal:
            names = ("causal_propagation_features", "causal_propagation_valid", "cp_valid_window_fraction", "cp_valid_seizure_count", "cp_mean_var_stability")
            missing = [name for name in names if name not in batch]
            if missing:
                raise KeyError(f"P23 P6 requires causal quality fields: {missing}")
            valid = batch["causal_propagation_valid"].bool() & channel_mask
            coverage = batch["cp_valid_window_fraction"].clamp(0.0, 1.0)
            seizure_quality = (batch["cp_valid_seizure_count"] / 3.0).clamp(0.0, 1.0)
            stability = (batch["cp_mean_var_stability"] / 0.50).clamp(0.0, 1.0)
            q_cp = (valid.to(direct_logits.dtype) * coverage * seizure_quality * stability).masked_fill(~valid, 0.0)
            raw = self.p23_causal_head(cp_features).squeeze(-1).masked_fill(~valid, 0.0)
            causal = {
                "u_causal": q_cp * raw, "raw_u_causal": raw, "q_cp": q_cp,
                "causal_feature_valid": valid, "causal_residual": zeros,
                "causal_residual_saturation_rate": direct_logits.sum() * 0.0,
            }
        elif self.use_causal_propagation_residual:
            cp_valid = batch.get("causal_propagation_valid")
            if cp_valid is None:
                cp_valid = torch.zeros_like(channel_mask)
            causal = self.causal_propagation_residual(
                cp_features, cp_valid.to(dtype=torch.bool), channel_mask
            )
            causal.update({"u_causal": zeros, "raw_u_causal": zeros, "q_cp": zeros})
        seizure_term = 0.20 * tail["u_seizure"]
        if self.p23_profile.bounded_fusion:
            fusion = self.p23_fusion(
                patient_embedding, direct_logits, anchor["u_anchor"], tail["u_seizure"],
                tail["seizure_agreement"], temporal_delta_norm, channel_mask,
                u_causal=causal["u_causal"], q_cp=causal["q_cp"],
            )
            # Legacy P2 causal residual, when retained for a regression
            # comparison, is added once here. The fusion delta never includes it.
            final_logits = (fusion["final_nez_logit"] + causal["causal_residual"]).masked_fill(~channel_mask, -1e9)
            fusion["final_nez_logit"] = final_logits
            fusion["final_score_nez"] = torch.sigmoid(final_logits).masked_fill(~channel_mask, 0.0)
            fusion["final_score_ez"] = (1.0 - fusion["final_score_nez"]).masked_fill(~channel_mask, 0.0)
        else:
            legacy_seizure = self.multi_seizure_evidence(
                seizure_embedding, batch["seizure_mask"], seizure_channel_mask
            )
            seizure_term = legacy_seizure["seizure_residual"] if not self.p23_profile.seizure_tail else seizure_term
            final_logits = (direct_logits + anchor["anchor_residual"] + seizure_term + causal["causal_residual"]).masked_fill(~channel_mask, -1e9)
            fusion = {"r_direct": zeros, "w_noop": torch.ones_like(zeros), "w_anchor": zeros, "w_seizure": zeros,
                      "w_causal": zeros, "delta": (final_logits - direct_logits).masked_fill(~channel_mask, 0.0),
                      "final_nez_logit": final_logits, "final_score_nez": torch.sigmoid(final_logits).masked_fill(~channel_mask, 0.0),
                      "final_score_ez": (1.0 - torch.sigmoid(final_logits)).masked_fill(~channel_mask, 0.0),
                      "fusion_entropy": zeros, "correction_saturation": torch.zeros_like(channel_mask)}
        output: dict[str, Any] = {
            "logits": final_logits, "scores": fusion["final_score_nez"], "score_nez": fusion["final_score_nez"],
            "score_ez": fusion["final_score_ez"], "direct_nez_logit": direct_logits,
            "direct_nez_logits": direct_logits, "direct_score_nez": direct["score_nez"],
            "direct_score_ez": direct["score_ez"], "final_nez_logit": final_logits,
            "final_nez_logits": final_logits, "final_score_nez": fusion["final_score_nez"],
            "final_score_ez": fusion["final_score_ez"], "patient_channel_embedding": patient_embedding,
            "contextual_channel_embedding": direct["contextual_channel_embedding"],
            "seizure_channel_embedding": seizure_embedding, "channel_mask": channel_mask,
            "causal_propagation_features": cp_features,
            "temporal_attention": temporal_weights, "seizure_attention": seizure_weights,
            "decision_rule": "inner_oof_global_nez_probability_threshold",
        }
        output.update(anchor); output.update(tail); output.update(temporal); output.update(temporal_patient); output.update(causal); output.update(fusion); output.update(physics_losses)
        output["seizure_term"] = seizure_term
        output["temporal_delta_norm"] = temporal_delta_norm
        output["physics_gate_mean"] = physics_gate.mean()
        require_finite("final_score_nez", output["final_score_nez"], channel_mask)
        return output

    def _forward_cane_set_nez(self, batch: dict[str, Any]) -> dict[str, Any]:
        """Run the feature-only CANE path without consuming labels or true counts."""
        ...
        if self.use_physics_dynamics:
            if "physics_features" not in batch:
                raise KeyError("CANE batch is missing physics_features")
            h_dyn, physics_losses = self.physics_encoder(
        ...
        direct_output = self.channel_classifier(patient_embedding, channel_mask)
        direct_logits = direct_output["logits"]
        contextual_embedding = direct_output["contextual_channel_embedding"]
        anchor_output = self.clean_nez_anchor(contextual_embedding, channel_mask)
        seizure_output = self.multi_seizure_evidence(
            seizure_embedding,
            batch["seizure_mask"],
            seizure_channel_mask,
        )
        final_logits = (
            direct_logits
            + anchor_output["anchor_residual"]
            + seizure_output["seizure_residual"]
        ).masked_fill(~channel_mask, -1e9)
        final_scores = self._scores_from_logits(final_logits, channel_mask)
        final_scores["score_ez"] = 1.0 - final_scores["score_nez"]
        cardinality_output = self.cardinality_head(
            contextual_embedding,
            final_logits,
            channel_mask,
            batch["seizure_mask"],
        )
        predicted_nez_mask = deterministic_topk_mask(
            final_scores["score_nez"], channel_mask, cardinality_output["predicted_nez_count"]
        )
        predicted_ez_mask = channel_mask & ~predicted_nez_mask
        output: dict[str, Any] = dict(final_scores)
        output.update(anchor_output)
        output.update(seizure_output)
        output.update(cardinality_output)
        output.update(branch_losses)
        output.update({
            "logits": final_logits,
            "scores": final_scores["score_nez"],
            "direct_nez_logits": direct_logits,
            "direct_score_nez": direct_output["score_nez"],
            "final_nez_logits": final_logits,
            "predicted_nez_mask": predicted_nez_mask,
            "predicted_ez_mask": predicted_ez_mask,
            "contextual_channel_embedding": contextual_embedding,
            "patient_channel_embedding": patient_embedding,
            "seizure_channel_embedding": seizure_embedding,
    ...
    def _forward_p21_v3_asrr_nez(self, batch: dict[str, Any]) -> dict[str, Any]:
        """One shared P2.1 path; profiles change only guarded components."""
        if self.p21_profile.legacy_p2:
            output = self._forward_cane_path_cp_nez(batch)
            output["p21_profile"] = self.p21_profile.name
            output["p21_legacy_p2_path"] = True
            return output
        channel_mask = batch["channel_mask"].bool()
        seizure_channel_mask = batch["seizure_channel_mask"].bool()
        h_b0 = self.b0_encoder(batch["b0_features"], None, seizure_channel_mask)
        if not self.use_physics_dynamics or "physics_features" not in batch:
            raise KeyError("P2.1 requires the Step4B physics feature branch")
        h_dyn, branch_losses = self.physics_encoder(
            batch["physics_features"], batch.get("window_mask"), seizure_channel_mask,
            batch.get("window_centers"),
        )
        physics_gate = torch.sigmoid(self.physics_gate(torch.cat((h_b0, h_dyn), dim=-1)))
        fused = self.m1_norm(h_b0 + physics_gate * h_dyn)
        ...
        direct_output = self.channel_classifier(patient_embedding, channel_mask)
        direct_logits = direct_output["logits"]
        contextual = direct_output["contextual_channel_embedding"]
        direct_standardized = robust_patient_standardize_v3(
            direct_logits, channel_mask, channel_mask
        )["v3_anchor_standardized"]

        if self.p21_profile.v3_anchor:
            required = ("v3_score_nez", "v3_logit_nez", "v3_anchor_valid")
            missing = [name for name in required if name not in batch]
            if missing:
                raise KeyError(f"{self.p21_profile.name} requires V3 anchor tensors: {missing}")
            v3 = robust_patient_standardize_v3(
                batch["v3_logit_nez"], channel_mask, batch["v3_anchor_valid"]
            )
            if torch.any(channel_mask & ~v3["v3_anchor_valid"]):
                raise RuntimeError("P2.1 V3 anchor is missing valid model channels; no direct fallback is allowed")
            base = v3["v3_anchor_standardized"]
        else:
            v3 = {
                "v3_anchor_standardized": torch.zeros_like(direct_logits),
                "v3_anchor_median": torch.zeros(direct_logits.shape[0], device=direct_logits.device),
                "v3_anchor_iqr": torch.zeros(direct_logits.shape[0], device=direct_logits.device),
                "v3_anchor_valid": torch.zeros_like(channel_mask),
            }
            base = direct_standardized

        anchor_output = self.clean_nez_anchor(contextual, channel_mask)
        seizure_output = self.multi_seizure_evidence.forward_p21(
            seizure_embedding, batch["seizure_mask"], seizure_channel_mask,
            use_q10=self.p21_profile.q10_seizure,
        )
        cp_features = batch.get("causal_propagation_features")
        cp_valid = batch.get("causal_propagation_valid")
        quality_names = ("cp_valid_window_fraction", "cp_valid_seizure_count", "cp_mean_var_stability")
        if cp_features is None or cp_valid is None or any(name not in batch for name in quality_names):
            raise KeyError("P2.1 bounded fusion requires causal features and all deterministic quality fields")
        causal_output = self.causal_propagation_residual.forward_p21(
            cp_features, cp_valid, channel_mask,
            batch["cp_valid_window_fraction"], batch["cp_valid_seizure_count"],
            batch["cp_mean_var_stability"],
        )
        fusion = self.p21_selective_fusion(
            contextual, base, direct_logits, anchor_output["u_anchor"],
            seizure_output["u_seizure"], causal_output["u_causal"], causal_output["q_cp"],
            seizure_output["seizure_nez_agreement"], channel_mask,
        )
        final_logits = fusion["final_nez_logit"]
        score_nez = torch.sigmoid(final_logits).masked_fill(~channel_mask, 0.0)
        output: dict[str, Any] = {
            "logits": final_logits, "scores": score_nez,
            "score_nez": score_nez, "score_ez": (1.0 - score_nez).masked_fill(~channel_mask, 0.0),
            "direct_nez_logit": direct_logits, "direct_nez_logits": direct_logits,
            "direct_score_nez": direct_output["score_nez"],
            "final_nez_logits": final_logits, "final_score_nez": score_nez,
            "final_score_ez": (1.0 - score_nez).masked_fill(~channel_mask, 0.0),
            "patient_channel_embedding": patient_embedding,
            "contextual_channel_embedding": contextual,
            "seizure_channel_embedding": seizure_embedding,
            "causal_propagation_features": cp_features,
            "cp_early_source_rank": cp_features[..., 3],
            "channel_mask": channel_mask,
            "temporal_attention": temporal_weights, "seizure_attention": seizure_weights,
            "physics_gate_mean": physics_gate.mean(),
            "decision_rule": "p21_global_probability_threshold",
            "p21_profile": self.p21_profile.name,
            "p21_legacy_p2_path": False,
        }
        output.update(branch_losses)
        output.update(v3)
        output.update(anchor_output)
        output.update(seizure_output)
        output.update(causal_output)
        output.update(fusion)
        output["causal_residual"] = torch.zeros_like(final_logits)
        output["causal_residual_saturation_rate"] = final_logits.sum() * 0.0
        return output

    def _forward_cane_path_cp_nez(self, batch: dict[str, Any]) -> dict[str, Any]:
        """Feature-only N8F ranking path; labels, center and thresholds are not inputs."""
        channel_mask = batch["channel_mask"].to(dtype=torch.bool)
        seizure_channel_mask = batch["seizure_channel_mask"].to(dtype=torch.bool)
        h_b0 = self.b0_encoder(batch["b0_features"], None, seizure_channel_mask)
        branch_losses: dict[str, torch.Tensor] = {}
        if not self.use_physics_dynamics or "physics_features" not in batch:
            raise KeyError("CANE-PATH-CP requires the Step4B physics static feature branch")
        h_dyn, physics_losses = self.physics_encoder(
            batch["physics_features"], batch.get("window_mask"), seizure_channel_mask,
            batch.get("window_centers"),
        )
        ...
        profile = str(getattr(self.args, "cane_profile", "P2")).upper()
        if profile == "P3":
            for field in ("anchor_residual", "anchor_distance", "anchor_distance_z", "anchor_nez_evidence"):
                anchor_output[field] = torch.zeros_like(anchor_output[field])
        if profile == "P4":
            for field in (
                "seizure_residual", "seizure_nez_logit", "seizure_nez_probability",
                "seizure_nez_probability_mean", "seizure_nez_probability_std",
                "seizure_nez_agreement",
            ):
                seizure_output[field] = torch.zeros_like(seizure_output[field])
        cp_features = batch.get("causal_propagation_features")
        cp_valid = batch.get("causal_propagation_valid")
        if cp_features is None:
            cp_features = torch.zeros((*channel_mask.shape, 6), device=direct_logits.device, dtype=direct_logits.dtype)
        if cp_valid is None:
            cp_valid = torch.zeros_like(channel_mask)
        if not self.use_causal_propagation_residual:
            cp_valid = torch.zeros_like(cp_valid, dtype=torch.bool)
        causal_output = self.causal_propagation_residual(cp_features, cp_valid, channel_mask)
        final_logits = (
            direct_logits + anchor_output["anchor_residual"] + seizure_output["seizure_residual"]
            + causal_output["causal_residual"]
        ).masked_fill(~channel_mask, -1e9)
        score_nez = torch.sigmoid(final_logits).masked_fill(~channel_mask, 0.0)
        ...
        output: dict[str, Any] = {
            ...
            "score_nez": score_nez,
            "score_ez": score_ez,
            "direct_nez_logit": direct_logits,
            "direct_nez_logits": direct_logits,
            "direct_score_nez": direct_output["score_nez"],
            "final_nez_logit": final_logits,
            "final_nez_logits": final_logits,
