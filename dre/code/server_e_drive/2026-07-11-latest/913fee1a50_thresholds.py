...
def select_patient_macro_threshold(table: pd.DataFrame, *, source: str = "inner_oof_patient_macro_f1") -> Task1Threshold:
    ...
    candidates = []
    for threshold in np.round(np.arange(0.0, 1.0001, 0.005), 3):
    ...
    best = max(candidates)
    return Task1Threshold(best[-1], best[0], best[1], best[2], source=source)
