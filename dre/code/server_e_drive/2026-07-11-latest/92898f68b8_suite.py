...
def run_task2_baseline_suite(
    *,
    ...
    old90_table = pd.read_csv(old90_manifest_path)
    if "subject_id" not in old90_table:
        raise ValueError("Task 1 old-90 manifest must contain subject_id.")
    old90 = set(old90_table["subject_id"].astype(str))
    cohort, cohort_audit = build_task2_primary_cohort(outcome_manifest, old90_subjects=old90)
    ...
    selected_examples = [example for example in all_examples if example.subject_id in set(cohort["subject_id"])]
    ...
    folds = build_composite_fold_ledger(cohort, n_splits=5, seed=42, cohort="task2_primary")
    ...
    provenance = {
        ...
        "label_semantics": {"success": 1, "failure": 0},
    ...
    config_hash = hashlib.sha256(json.dumps({"models": requested, "seeds": list(seeds), "inner_folds": inner_folds, "compact": compact}, sort_keys=True).encode("utf-8")).hexdigest()
    for model in requested:
        if model not in SIMPLE_MODELS:
            continue
        for seed in seeds:
            try:
                result = run_simple_baseline_oof(summary, folds, model_name=model, seed=int(seed), inner_folds=inner_folds, compact=compact)
                ...
                result.oof["cohort_name"] = "task2_primary_old90_success_plus_valid_failure"
        ...
        for seed in seeds:
            try:
                loco = run_simple_baseline_loco(
                    summary, model_name=model, seed=int(seed), inner_folds=inner_folds, compact=compact
                )
    ...
    if neural:
        ...
        if not neural_result.predictions.empty:
            for (variant, current_seed), frame in neural_result.predictions.groupby(["variant", "seed"]):
                ...
                normalized["cohort_name"] = "task2_primary_old90_success_plus_valid_failure"
    ...
    for alias, variant in RAW_ALIASES.items():
        ...
        for seed in seeds:
            try:
                ...
                frame["cohort_name"] = "task2_primary_old90_success_plus_valid_failure"
