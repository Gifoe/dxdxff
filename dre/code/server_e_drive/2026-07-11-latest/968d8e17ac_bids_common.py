...
def _parse_outcome_success_value(value: Any) -> bool | None:
    parsed = is_successful_surgery_value(value)
    ...
    if text_compact in {"seizurefree", "szfree", "freeofseizures", "recurrencefree"}:
        return True
    if "seizure free" in text or "free of seizures" in text:
        return True
    if text_compact in {"recurrence", "recurrent", "relapse", "failed", "failure"}:
        return False
    if any(token in text for token in ("recurrence", "recurrent", "relapse")):
        return False
    return None


def participant_surgery_success_with_source(participant_meta: Mapping[str, Any] | None) -> tuple[bool | None, str | None]:
    if not participant_meta:
        return None, None

    if "outcome" in participant_meta:
        parsed = _parse_outcome_success_value(participant_meta.get("outcome"))
        if parsed is not None:
            return parsed, "outcome"

    for key in ("outcome_binary", "success", "surgery_success", "seizure_free"):
        if key not in participant_meta:
            continue
        binary = as_binary(participant_meta.get(key))
        if binary is not None:
            return bool(binary), key
        parsed = _parse_outcome_success_value(participant_meta.get(key))
        if parsed is not None:
            return parsed, key

    for key in ("engel_score", "engel"):
        if key not in participant_meta:
            continue
        engel_numeric = safe_float(participant_meta.get(key))
        if engel_numeric is not None:
            if engel_numeric == 1.0:
                return True, key
            if engel_numeric in {2.0, 3.0, 4.0}:
                return False, key
        parsed = is_successful_surgery_value(participant_meta.get(key))
        if parsed is not None:
            return parsed, key

    for key in ("ilae_score", "ilae"):
        if key not in participant_meta:
            continue
        ilae = safe_float(participant_meta.get(key))
        if ilae is not None:
            return ilae == 1.0, key
...
def participant_surgery_success(participant_meta: Mapping[str, Any] | None) -> bool | None:
    success, _ = participant_surgery_success_with_source(participant_meta)
    return success
