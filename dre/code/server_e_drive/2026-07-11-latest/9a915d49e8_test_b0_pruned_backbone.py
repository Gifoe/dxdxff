...
class B0PrunedBackboneTests(unittest.TestCase):
    ...
    def test_old_window_cache_schema_builds_pruned_batch_without_diffusion_view_by_default(self):
        sample = _sample("p1")
        args = _args()
        ...
        self.assertNotIn("diffusion_adjacency", batch)
    ...
    def test_diffusion_batch_includes_aligned_adjacency_only_when_enabled(self):
        sample = _sample("p1")
        args = _args(use_diffusion_residual=True)
        normalizer = fit_window_tensor_normalizer([sample], args=args)

        examples = build_patient_examples([sample], _patient_index("p1"), normalizer=normalizer, args=args)
        batch = collate_patient_ez_batch(examples)

        self.assertIn("diffusion_adjacency", batch)
        self.assertEqual(batch["diffusion_adjacency"].shape[-2:], (3, 3))

    def test_invalid_adjacency_shape_falls_back_to_zero_tensor(self):
        sample = _sample("p1")
        sample["window_adjacency"] = np.ones((4, 2, 2), dtype=np.float32)
        args = _args(use_diffusion_residual=True)
        normalizer = fit_window_tensor_normalizer([sample], args=args)
        ...
        self.assertIn("diffusion_adjacency", batch)
        self.assertEqual(float(batch["diffusion_adjacency"].abs().sum().item()), 0.0)
