...
def build_task2_primary_cohort(
    outcome_manifest: pd.DataFrame,
    *,
    old90_subjects: Iterable[str],
    ...
    if positive_class not in {"success", "failure"}:
        raise ValueError("Task 2 positive_class must be 'success' or 'failure'.")
    ...
    cohort["outcome_label"] = (cohort["outcome_group"] == positive_class).astype(int)
