...
def test_ledger_writer_accepts_summary_list_masks(tmp_path) -> None:
    ...
    _write_ledgers(enriched, tmp_path, fold=1, seed=42, split_role="validation")
    assert (tmp_path / "val_patient_predictions_neuroez_v2_fold_1.csv").is_file()
    assert (tmp_path / "val_channel_predictions_neuroez_v2_fold_1.csv").is_file()
