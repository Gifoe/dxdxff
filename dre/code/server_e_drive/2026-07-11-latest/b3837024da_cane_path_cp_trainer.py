...
def _loader(experiment: Any, dataset: Any, *, train: bool, seed: int) -> DataLoader:
    if train and bool(getattr(experiment.args, "center_balanced_batches", False)):
        ...
        return DataLoader(
            ...
            collate_fn=experiment.runtime["collate_fn"], pin_memory=experiment.device.type == "cuda",
        )
    return DataLoader(
...
def _optimizer(model: torch.nn.Module, args: Any) -> torch.optim.Optimizer:
    head_names = ("clean_nez_anchor", "multi_seizure_evidence", "causal_propagation_residual")
    backbone, heads = [], []
...
@torch.no_grad()
def collect_ranking_records(experiment: Any, model: torch.nn.Module, loader: DataLoader, *, source: str) -> list[dict[str, Any]]:
    model.eval()
    ...
    for batch in loader:
        ...
        for patient_idx, subject_id in enumerate(batch["subject_id"]):
            ...
            for name in (
                ...
                "u_anchor", "u_seizure", "raw_u_causal", "u_causal", "q_cp",
...
def ranking_validation_summary(records: Sequence[dict[str, Any]]) -> dict[str, float]:
    ...
    for threshold in np.linspace(0.05, 0.95, 37):
        patient_macro_f1, patient_nez_f1 = [], []
        for record in records:
    ...
    best_macro_f1, best_nez_f1, best_threshold = max(
        threshold_rows,
        key=lambda row: (row[0], row[1], -abs(row[2] - 0.5)),
    )
    return {
...
def _write_ledgers(
    records: Sequence[dict[str, Any]], output_dir: Path, fold: int, seed: int,
    *, split_role: str = "test",
) -> None:
    if split_role not in {"validation", "test"}:
        raise ValueError("split_role must be validation or test")
    prefix = "val" if split_role == "validation" else "test"
    ...
    pd.DataFrame(patient_rows).to_csv(output_dir / f"{prefix}_patient_predictions_neuroez_v2_fold_{fold}.csv", index=False)
    pd.DataFrame(channel_rows).to_csv(output_dir / f"{prefix}_channel_predictions_neuroez_v2_fold_{fold}.csv", index=False)
...
def run_cane_path_cp_outer_only(experiment: Any) -> list[dict[str, Any]]:
    ...
    for split in splits:
        ...
        fold_outputs = (
            output / f"val_patient_predictions_neuroez_v2_fold_{outer_fold}.csv",
            output / f"val_channel_predictions_neuroez_v2_fold_{outer_fold}.csv",
        ...
        if not fit_subjects or not validation_subjects:
        ...
        model, best_epoch, selected_threshold, history, prototype_audit, validation_loader = train_ranking_model(
            experiment,
            fit_subjects,
            validation_subjects,
            fold_tag=tag,
        ...
        frozen_validation = collect_ranking_records(
            experiment, model, validation_loader, source=f"outer{outer_fold}_validation",
        )
        predicted_validation = apply_fixed_nez_probability_threshold(
            frozen_validation, selected_threshold,
            threshold_source="fold_validation_macro_f1",
        )
        _write_ledgers(
            predicted_validation, output, outer_fold, seed,
        ...
        _write_ledgers(enriched, output, outer_fold, seed, split_role="test")
        audit = {
        ...
        del model, validation_loader, frozen_validation, predicted_validation, test_loader, frozen_test, predicted, enriched
        gc.collect()
        if experiment.device.type == "cuda":
