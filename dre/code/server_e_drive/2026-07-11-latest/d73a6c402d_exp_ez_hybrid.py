...
from neuroez_c.v3_qbc_loss import compute_v3_qbc_loss
from neuroez_c.v3_qbc_profiles import get_v3_qbc_profile
from neuroez_c.v3_rcc_loss import rcc_loss_components
from neuroez_c.v3_rcc_profiles import get_v3_rcc_profile
from neuroez_c.v3_rcc_decoder import fit_raw_probability_threshold
...
def _move_tensors_to_device(batch: Dict[str, Any], device: torch.device) -> Dict[str, Any]:
    ...
    return {
        # Raw waveforms can be several GiB after padded patient collation.
        # RawWindowEncoder streams only valid chunks to CUDA instead of
        # materialising this complete tensor on the GPU.
        key: (
            value
            if key == "raw_windows"
...
def _quality_audit_frames(
    ...
    for split in outer_splits:
        ...
        fit_subjects = list(split.get("fit_subjects", []))
        val_subjects = list(split.get("validation_subjects", []))
...
def _v3_qbc_checkpoint_components(records: Sequence[Dict[str, Any]]) -> Dict[str, float]:
    """Threshold-free validation components; true-K is diagnostic selection only."""
    truek_f1: list[float] = []
    for record in records:
        valid = np.asarray(record["channel_mask"], dtype=bool)
        labels_ez = np.asarray(record["labels_ez"], dtype=np.int64)[valid]
        # A0_BASE has no Q10 residual, and older evaluation records can omit
        # final_nez_logit. score_nez=P(NEZ) has the identical patient-wise
        # ordering, which is all this true-K *diagnostic* needs.
        nez_rank_source = record.get("final_nez_logit")
        if nez_rank_source is None:
            nez_rank_source = record.get("base_nez_logit")
        if nez_rank_source is None:
            nez_rank_source = record.get("score_nez", record.get("scores"))
        final_nez_all = np.asarray(nez_rank_source, dtype=np.float64)
        if final_nez_all.ndim != 1 or final_nez_all.shape != valid.shape:
            continue
        final_nez = final_nez_all[valid]
        if labels_ez.size == 0 or not np.isfinite(final_nez).all():
            continue
        k = int(labels_ez.sum())
        pred_ez = np.zeros(labels_ez.size, dtype=np.int64)
        pred_ez[np.argsort(final_nez, kind="stable")[:k]] = 1
        truek_f1.append(
            float(f1_score(1 - labels_ez, 1 - pred_ez, labels=[0, 1], average="macro", zero_division=0))
        )
    summary, _ = _summarize_prediction_records(records)
    ez_auprc = float(summary.get("patient_macro_auprc_ez", 0.0))
    ez_mrr = float(summary.get("patient_macro_ez_mrr", 0.0))
    truek = float(np.mean(truek_f1)) if truek_f1 else 0.0
    return {
        "validation_patient_ez_auprc": ez_auprc,
        "validation_patient_ez_mrr": ez_mrr,
        "validation_true_k_macro_f1_diagnostic": truek,
        "rank_score": 0.45 * ez_auprc + 0.25 * ez_mrr + 0.30 * truek,
    }


def _v3_rcc_checkpoint_components(records: Sequence[Dict[str, Any]]) -> Dict[str, float]:
    """Validation-only RCC selection, including a raw-probability threshold fit."""
    threshold_fit = fit_raw_probability_threshold(list(records))
    formal, _ = _summarize_prediction_records(records, classification_threshold=float(threshold_fit["threshold"]))
    diagnostic = _v3_qbc_checkpoint_components(records)
    rank_score = (
        0.40 * float(diagnostic["validation_patient_ez_auprc"])
        + 0.25 * float(diagnostic["validation_patient_ez_mrr"])
        + 0.20 * float(diagnostic["validation_true_k_macro_f1_diagnostic"])
        + 0.15 * float(formal.get("patient_macro_f1", 0.0))
        - 0.05 * float(formal.get("center_gap_f1", 0.0))
    )
    return {
        "val_raw_threshold_macro_f1": float(formal.get("patient_macro_f1", 0.0)),
        "val_ez_auprc": float(diagnostic["validation_patient_ez_auprc"]),
        "val_ez_mrr": float(diagnostic["validation_patient_ez_mrr"]),
        "val_truek_macro_f1": float(diagnostic["validation_true_k_macro_f1_diagnostic"]),
        "val_center_gap": float(formal.get("center_gap_f1", 0.0)),
        "val_raw_threshold": float(threshold_fit["threshold"]),
...
class Exp_EZHybridLocalization:
    ...
    def __init__(self, args: Any) -> None:
        ...
        self.use_v3_qbc = bool(getattr(args, "use_v3_qbc", False))
        self.v3_qbc_profile = get_v3_qbc_profile(getattr(args, "v3_qbc_profile", "BCR_BC_ONLY"))
        self.use_v3_rcc = bool(getattr(args, "use_v3_rcc", False))
        self.v3_rcc_profile = get_v3_rcc_profile(getattr(args, "v3_rcc_profile", "R0_BASE"))
    ...
    def _compute_loss(
        ...
        *,
        ...
        use_v3_rcc = bool(getattr(self, "use_v3_rcc", False))
        if use_v3_rcc:
            bce, supervised_parts = _patient_balanced_bce_loss(outputs["logits"], batch, self.args)
            supervised_parts["loss_mode"] = "patient_balanced_bce"
        elif bool(getattr(self, "use_broad_ez_mil_loss", False)):
        ...
        if use_v3_rcc:
            rcc_classification, rcc_parts = rcc_loss_components(
                outputs, batch, bce, self.args, epoch=int(getattr(self, "current_epoch", 1))
            )
            loss = rcc_classification
        else:
            rcc_parts = {}
            loss = bce
        zero = bce.sum() * 0.0
        ...
        if negative_anchor_enabled:
            anchor_weight = (
                float(getattr(self.args, "v3_qbc_anchor_weight", 0.02))
                if bool(getattr(self, "use_v3_qbc", bool(getattr(self.args, "use_v3_qbc", False))))
                else float(getattr(self.args, "negative_anchor_loss_weight", 0.03))
            )
            if use_v3_rcc:
                anchor_weight = float(getattr(self.args, "v3_rcc_anchor_weight", 0.02))
            loss = loss + anchor_weight * negative_anchor
        ...
        ranking_weight = float(getattr(self.args, "v3_rcc_pairwise_weight", 0.05)) if use_v3_rcc else float(getattr(self.args, "ez_ranking_loss_weight", 0.0))
        ...
        if ranking_enabled and ranking_weight > 0.0:
            ...
            ez_ranking = zero
        use_v3_qbc = bool(getattr(self, "use_v3_qbc", bool(getattr(self.args, "use_v3_qbc", False))))
        v3_qbc_profile = getattr(
            self,
            "v3_qbc_profile",
            get_v3_qbc_profile(getattr(self.args, "v3_qbc_profile", "BCR_BC_ONLY")),
        )
        if use_v3_qbc and (v3_qbc_profile.use_boundary or v3_qbc_profile.use_coverage):
            v3_qbc_aux, v3_qbc_parts = compute_v3_qbc_loss(outputs, batch, self.args)
            loss = loss + v3_qbc_aux
        else:
            v3_qbc_aux = zero
            v3_qbc_parts = {}
        ...
        if use_v3_qbc:
            parts["v3_qbc_aux_loss"] = float(v3_qbc_aux.detach().cpu())
            for key, value in v3_qbc_parts.items():
                parts[key] = float(value.detach().cpu()) if torch.is_tensor(value) else float(value)
        if use_v3_rcc:
            for key, value in rcc_parts.items():
                parts[key] = float(value.detach().cpu()) if torch.is_tensor(value) else float(value)
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                for key in (
                    ...
                    "base_nez_logit", "final_nez_logit", "ez_semantic_logit",
                    "valid_seizure_count", "seizure_nez_probability_mean",
                    "seizure_nez_probability_std",
                ):
                ...
                for qbc_key in (
                    "v3_qbc_aux_loss", "boundary_loss", "mean_hard_positive_score",
                    "mean_hard_negative_score", "boundary_margin_observed",
                    "n_boundary_valid_patients", "soft_topk_coverage_loss",
                    "soft_topk_mean_coverage", "n_coverage_valid_patients",
                ):
                    if qbc_key in loss_parts:
                        optional_scalars[qbc_key] = float(loss_parts[qbc_key])
                for broad_key in (
                    "broad_ez_mil_loss",
                    "broad_ez_mil_loss_weight",
                    "broad_ez_core_frac",
        ...
        for record in records:
            ...
            for channel_idx, channel_name in enumerate(record["canonical_channels"]):
                channel_dict = {
                        ...
                        # Persist frozen contextual features for a later bounded adapter.
                        "contextual_embedding": json.dumps(
                            np.asarray(record.get("patient_channel_embedding", np.empty((0, 0))))[channel_idx].astype(float).tolist()
                        ) if "patient_channel_embedding" in record else "",
    ...
    def run(self) -> List[Dict[str, Any]]:
        ...
        v3_qbc_checkpoint_rows: List[Dict[str, Any]] = []
        v3_qbc_train_loss_rows: List[Dict[str, Any]] = []
        v3_rcc_checkpoint_rows: List[Dict[str, Any]] = []
        v3_rcc_train_loss_rows: List[Dict[str, Any]] = []
        ...
        for split in outer_splits:
            ...
            best_model_path = (
                fold_dir / "best_bcr_boundary_coverage.pt"
                if self.use_v3_qbc
                else fold_dir / "best_b0_pruned_model.pth"
            )
            v3_qbc_fold_history: list[dict[str, float]] = []
            v3_rcc_fold_history: list[dict[str, float]] = []

            for epoch in range(1, epochs + 1):
                self.current_epoch = epoch
                train_metrics = self._train_one_epoch(model, train_loader, optimizer, ez_negative_weight, ema_teacher=ema_teacher)
                if self.use_v3_qbc:
                    v3_qbc_train_loss_rows.append(
                        {
                ...
                if self.use_v3_rcc:
                    v3_rcc_train_loss_rows.append({"outer_fold": int(fold_idx), "epoch": int(epoch), **{key: value for key, value in train_metrics.items() if isinstance(value, (int, float))}})
                if self.use_a9v8_lcbo:
                    train_row = {
                        "fold_id": int(fold_idx),
                        "split": "train",
                ...
                if self.use_v3_qbc:
                    qbc_components = _v3_qbc_checkpoint_components(raw_val_records)
                    v3_qbc_fold_history.append(qbc_components)
                    rolling = v3_qbc_fold_history[-3:]
                    rolling_score = float(np.mean([row["rank_score"] for row in rolling]))
                    checkpoint_min_epoch = max(3, int(getattr(self.args, "v3_qbc_checkpoint_min_epoch", 3)))
                    val_score = rolling_score if epoch >= checkpoint_min_epoch else -float("inf")
                    v3_qbc_checkpoint_rows.append(
                        {
                            "outer_fold": int(fold_idx),
                            "epoch": int(epoch),
                            **qbc_components,
                            "rolling_rank_score_3epoch": rolling_score,
                            "checkpoint_eligible": bool(epoch >= checkpoint_min_epoch),
                        }
                    )
                    val_summary.update(qbc_components)
                    val_summary["rolling_rank_score_3epoch"] = rolling_score
                elif self.use_v3_rcc:
                    rcc_components = _v3_rcc_checkpoint_components(raw_val_records)
                    v3_rcc_fold_history.append(rcc_components)
                    rolling = v3_rcc_fold_history[-3:]
                    rolling_score = float(np.mean([row["rank_score"] for row in rolling]))
                    checkpoint_min_epoch = max(3, int(getattr(self.args, "v3_rcc_checkpoint_min_epoch", 3)))
                    val_score = rolling_score if epoch >= checkpoint_min_epoch else -float("inf")
                    v3_rcc_checkpoint_rows.append({
                        "outer_fold": int(fold_idx), "epoch": int(epoch), **rcc_components,
                        "val_rank_score_rolling3": rolling_score,
                        "selected_checkpoint": False,
                        "checkpoint_eligible": bool(epoch >= checkpoint_min_epoch),
                        "selected_reason": "validation_only_rolling3_rcc_rank_score",
                    })
                    val_summary.update(rcc_components)
                    val_summary["rolling_rank_score_3epoch"] = rolling_score
                else:
                ...
                if improved:
                    if self.use_v3_rcc and v3_rcc_checkpoint_rows:
                        v3_rcc_checkpoint_rows[-1]["selected_checkpoint"] = True
            ...
            if self.use_v3_qbc or self.use_v3_rcc:
                if not best_model_path.exists():
                    raise RuntimeError(f"V3-QBC best checkpoint was not saved for fold {fold_idx}")
                # V3-QBC checkpoints include trusted NumPy metadata in addition to tensors.
                # PyTorch 2.6 defaults weights_only=True, which rejects that metadata.
                disk_checkpoint = torch.load(
                    best_model_path,
                    map_location=self.device,
                    weights_only=False,
                )
                model.load_state_dict(disk_checkpoint["model_state_dict"])
            else:
                model.load_state_dict(best_state)
            if self.use_v3_rcc:
                # Freeze the raw-probability threshold using only this outer fold's validation patients.
                _, _, rcc_val_raw_records = self._evaluate(model, val_loader, ez_negative_weight, split_name="val")
                rcc_threshold = fit_raw_probability_threshold(list(rcc_val_raw_records))
                best_classification_threshold = float(rcc_threshold["threshold"])
                _, rcc_val_records = _summarize_prediction_records(
                    rcc_val_raw_records, classification_threshold=best_classification_threshold
                )
                self._save_outputs(rcc_val_records, fold_idx=fold_idx, split_name="val")
                v3_rcc_checkpoint_rows.append({
                    "outer_fold": int(fold_idx), "epoch": int(best_epoch),
                    **_v3_rcc_checkpoint_components(rcc_val_raw_records),
                    "selected_checkpoint": True,
                    "selected_reason": "restored_best_checkpoint_validation_only_raw_threshold",
                })
            if self.use_n6_dual_view_ema:
            ...
            if self.use_v3_rcc:
                # Adapter stage consumes only outer-train predictions from this frozen fold checkpoint.
                _, _, rcc_fit_raw_records = self._evaluate(model, train_loader, ez_negative_weight, split_name="fit")
                _, rcc_fit_records = _summarize_prediction_records(
                    rcc_fit_raw_records, classification_threshold=best_classification_threshold
                )
                self._save_outputs(rcc_fit_records, fold_idx=fold_idx, split_name="fit")
        ...
        if all_test_records:
            ...
            if v3_qbc_checkpoint_rows:
                pd.DataFrame(v3_qbc_checkpoint_rows).to_csv(
                    output_dir / "checkpoint_selection_by_epoch.csv", index=False
                )
            if v3_qbc_train_loss_rows:
                pd.DataFrame(v3_qbc_train_loss_rows).to_csv(
                    output_dir / "training_loss_components.csv", index=False
                )
            if v3_rcc_checkpoint_rows:
                pd.DataFrame(v3_rcc_checkpoint_rows).to_csv(output_dir / "checkpoint_selection_by_epoch.csv", index=False)
            if v3_rcc_train_loss_rows:
                pd.DataFrame(v3_rcc_train_loss_rows).to_csv(output_dir / "training_loss_components.csv", index=False)
            for key in (
                "positive_label",
                "score_semantics",
