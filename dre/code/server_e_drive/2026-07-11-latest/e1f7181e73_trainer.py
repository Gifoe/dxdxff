...
import copy, random, time, gc, math, numpy as np, torch
...
def batches(patients,size,seed):
 ...
 while pos or neg:
  b=[]
  while len(b)<size and (pos or neg):
   if pos and len(b)<size:b.append(pos.pop())
   if neg and len(b)<size:b.append(neg.pop())
  if b: out.append(b)
 sampled=[p.patient_key for batch in out for p in batch]
 expected=[p.patient_key for p in patients]
 assert len(sampled)==len(expected)
 assert len(set(sampled))==len(expected)
 assert set(sampled)==set(expected)
 assert len(out)==math.ceil(len(expected)/size)
 return out
...
def train_epoch(model,patients,opt,ema,device,seed,args,epoch):
 device=torch.device(device);model.train(); model.configure_encoder(args.current_train_window_batch_size,args.gradient_checkpoint_encoder,True); values=[]; opt.zero_grad(set_to_none=True); accumulation=int(args.gradient_accumulation_steps); pending=0; oom_backoffs=0; flat_windows=chunks=0; started=time.perf_counter();batch_plan=batches(patients,args.batch_size,seed);sampled_keys=[];stable_index={p.patient_key:i for i,p in enumerate(sorted(patients,key=lambda x:x.patient_key))};progress=tqdm(batch_plan,desc=f'Train epoch {epoch} batches',leave=False,disable=not getattr(args,'show_progress',True))
 for batch_index,batch in enumerate(progress):
  sampled_keys.extend(p.patient_key for p in batch);tensors=[patient_tensors(p,'cpu',np.random.default_rng(seed+stable_index[p.patient_key]),args.max_seizures_train,args.max_ez_channels_train,args.max_nez_channels_train,args.max_windows_per_phase_train) for p in batch]
  if any(not x for x in tensors): raise ValueError(f'empty sampled patient view in epoch {epoch}: {[p.patient_key for p,x in zip(batch,tensors) if not x]}')
  while True:
   try:
  ...
  batch_logits=torch.stack([x['logit'] for x in out]).detach();values.append({'total':float(loss.detach()),**{k:(float(v) if torch.is_tensor(v) else v) for k,v in parts.items()},'success_logit_mean':float(torch.stack([x['logit'] for x,p in zip(out,batch) if p.outcome_success]).detach().mean()) if any(p.outcome_success for p in batch) else np.nan,'failure_logit_mean':float(torch.stack([x['logit'] for x,p in zip(out,batch) if not p.outcome_success]).detach().mean()) if any(not p.outcome_success for p in batch) else np.nan,'logit_std':float(batch_logits.std(unbiased=False)),'gradient_norm':grad_norm});progress.set_postfix(bce=f'{float(parts["bce"]):.4f}',rank=f'{float(parts["rank_raw"]):.4f}',lambda_rank=f'{float(parts["rank_weight"]):.4f}',total_loss=f'{float(loss.detach()):.4f}',patients_seen=len(sampled_keys))
 if pending:
 ...
 expected_batches=math.ceil(len(patients)/args.batch_size);duplicates=len(sampled_keys)-len(set(sampled_keys));assert len(sampled_keys)==len(patients);assert len(set(sampled_keys))==len(patients);assert set(sampled_keys)=={p.patient_key for p in patients};assert len(batch_plan)==expected_batches;result={k:float(np.nanmean([x[k] for x in values])) for k in values[0]};result.update({'train_unique_patients':len(set(sampled_keys)),'train_sampled_rows':len(sampled_keys),'train_batches':len(batch_plan),'expected_train_batches':expected_batches,'duplicate_train_patients':duplicates,'oom_backoff_count':oom_backoffs,'n_flat_windows':flat_windows,'encoder_chunks':chunks,'windows_per_second':flat_windows/max(time.perf_counter()-started,1e-9),**memory_snapshot(device,args.vram_budget_fraction)});return result
...
def better_checkpoint(current_auroc, current_bce, best_auroc, best_bce, min_delta=1e-6):
 """Selection order is AUROC, then BCE only for an AUROC tie."""
 if not np.isfinite(current_auroc): return False
 return current_auroc > best_auroc + min_delta or (abs(current_auroc-best_auroc)<=min_delta and current_bce < best_bce-1e-4)
