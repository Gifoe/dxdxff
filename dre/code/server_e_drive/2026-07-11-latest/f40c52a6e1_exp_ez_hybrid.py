...
from neuroez_c.cane_set_heads import initialize_clean_nez_prototypes
from neuroez_c.cane_set_loss import cane_stage, compute_cane_set_loss
...
def _resolve_runtime(args: Any) -> Dict[str, Any]:
    ...
    return {
        "name": (
            "N7F-CANE-Set-NEZ"
            if bool(getattr(args, "use_cane_set_nez", False))
            else "A9v5-TwoExpert-NAREZ"
            if bool(getattr(args, "use_two_expert_router", False))
            else "B0-Pruned-EZBackbone"
        ),
...
def _summarize_prediction_records(
    ...
    for record in records:
        ...
        decision_rule = str(record.get("decision_rule", "")).strip().lower()
        explicit_mask_rules = {
            "predicted_nez_cardinality_topk",
            "patient_adaptive_standardized_nez_threshold",
            "fixed_nez_probability_threshold",
            PREDICTED_K_DECISION_RULE,
        }
        if decision_rule in explicit_mask_rules:
            if "predicted_nez_mask" not in record or "predicted_ez_mask" not in record:
                raise ValueError("CANE records require explicit predicted_nez_mask and predicted_ez_mask")
            pred_nez_mask = np.asarray(record["predicted_nez_mask"], dtype=bool)
            pred_ez_mask = np.asarray(record["predicted_ez_mask"], dtype=bool)
            if pred_nez_mask.shape != valid_mask.shape or pred_ez_mask.shape != valid_mask.shape:
                raise ValueError("CANE prediction masks must match channel_mask shape")
            if np.any(pred_nez_mask & pred_ez_mask) or not np.array_equal(
                (pred_nez_mask | pred_ez_mask), valid_mask
            ):
                raise ValueError("CANE NEZ/EZ prediction masks must be disjoint complements on valid channels")
            if np.any((pred_nez_mask | pred_ez_mask) & ~valid_mask):
                raise ValueError("CANE prediction masks selected invalid channels")
            if decision_rule == PREDICTED_K_DECISION_RULE:
                required = ("scope_predicted_k", "formal_prediction_source", "true_count_used_for_prediction")
                missing = [name for name in required if name not in record]
                if missing:
                    raise RuntimeError(f"SCOPE predicted-K record is missing {missing}")
                if int(pred_ez_mask.sum()) != int(record["scope_predicted_k"]):
                    raise RuntimeError("SCOPE predicted_ez_mask count does not equal scope_predicted_k")
                if int(pred_nez_mask.sum()) != int(valid_mask.sum() - int(record["scope_predicted_k"])):
                    raise RuntimeError("SCOPE predicted_nez_mask count is inconsistent with scope_predicted_k")
                if bool(record["true_count_used_for_prediction"]):
                    raise RuntimeError("SCOPE formal decoder must not use true count")
                if str(record["formal_prediction_source"]) != FORMAL_PREDICTION_SOURCE:
                    raise RuntimeError("SCOPE formal prediction source must be predicted_k_topk")
            threshold = (
                float(record.get("classification_threshold", 0.5))
                if decision_rule == "fixed_nez_probability_threshold"
                else float("nan")
            )
        else:
            threshold = float(
        ...
        patient_weighted_f1 = float(f1_score(y_nez, pred_nez, average="weighted", labels=[0, 1], zero_division=0))
        ...
        # Some callers carry batch-padded score/label arrays.  Channel names
        # are patient-local, so every exported channel list must be limited to
        # the canonical extent and valid patient channels.
        channel_extent = min(len(channel_names), int(valid_mask.shape[0]))
        export_valid = valid_mask[:channel_extent]

        def _named_channels(flags: np.ndarray) -> list[str]:
            local_flags = np.asarray(flags, dtype=bool)[:channel_extent]
            return [channel_names[idx] for idx in range(channel_extent) if export_valid[idx] and local_flags[idx]]
        ...
        enriched_record["threshold_source"] = "not_used" if decision_rule == PREDICTED_K_DECISION_RULE else (str(record.get("threshold_source", "not_used")) if decision_rule in explicit_mask_rules else str(record.get("threshold_source", "fixed")))
        enriched_record["decision_rule"] = decision_rule or "fixed_probability_threshold"
        enriched_record["true_count_used_for_prediction"] = False
        if decision_rule == PREDICTED_K_DECISION_RULE:
            enriched_record["formal_prediction_source"] = FORMAL_PREDICTION_SOURCE
        ...
        enriched_record["predicted_ez_channels"] = _named_channels(pred_ez_mask)
        enriched_record["predicted_nez_channels"] = _named_channels(pred_nez_mask)
        enriched_record["predicted_channels"] = enriched_record["predicted_ez_channels"]
        enriched_record["true_ez_channels"] = _named_channels(labels_ez == 1.0)
        enriched_record["true_nez_channels"] = _named_channels(labels_nez == 1.0)
        enriched_record["channel_name_alignment_ok"] = bool(
            len(channel_names) == len(labels_nez) == len(labels_ez) == len(score_nez)
        )
        ...
        true_nez_count = float(enriched_record["true_nez_count"])
        n_valid = max(int(valid_mask.sum()), 1)
        predicted_nez_fraction = float(record.get("predicted_nez_fraction", pred_nez_mask.sum() / n_valid))
        true_nez_fraction = true_nez_count / n_valid
        enriched_record["predicted_nez_fraction"] = predicted_nez_fraction
        enriched_record["expected_nez_count"] = float(record.get("expected_nez_count", pred_nez_mask.sum()))
        enriched_record["nez_count_error"] = float(abs(pred_nez_mask.sum() - true_nez_count))
        enriched_record["nez_fraction_error"] = float(abs(predicted_nez_fraction - true_nez_fraction))
        patient_metrics["nez_count_error"].append(enriched_record["nez_count_error"])
        patient_metrics["nez_fraction_error"].append(enriched_record["nez_fraction_error"])
        enriched.append(enriched_record)

    summary = {
        "classification_threshold": float("nan") if enriched and all(
            str(record.get("decision_rule", "")).lower() in {"predicted_nez_cardinality_topk", PREDICTED_K_DECISION_RULE} for record in enriched
        ) else float(
            classification_threshold
            if classification_threshold is not None
        ...
        "predicted_nez_count_mae": _safe_mean(patient_metrics["nez_count_error"]),
        "predicted_nez_fraction_mae": _safe_mean(patient_metrics["nez_fraction_error"]),
    }
    ...
    summary["nez_count_mae"] = summary["predicted_nez_count_mae"]
    summary["nez_fraction_mae"] = summary["predicted_nez_fraction_mae"]
    all_scope_predicted_k = bool(enriched) and all(str(record.get("decision_rule", "")).lower() == PREDICTED_K_DECISION_RULE for record in enriched)
    summary["decision_rule"] = PREDICTED_K_DECISION_RULE if all_scope_predicted_k else ("predicted_nez_cardinality_topk" if enriched and all(str(record.get("decision_rule", "")).lower() == "predicted_nez_cardinality_topk" for record in enriched) else "fixed_probability_threshold")
    summary["formal_prediction_source"] = FORMAL_PREDICTION_SOURCE if all_scope_predicted_k else "score_threshold"
    summary["threshold_source"] = "not_used" if summary["decision_rule"] in {"predicted_nez_cardinality_topk", PREDICTED_K_DECISION_RULE} else "fixed_or_validation"
    summary["true_count_used_for_prediction"] = False
...
def _summary_score(summary: Dict[str, float], args: Any, *, val_loss: float = 0.0) -> float:
    ...
    if metric == "cane_predcount_patient_macro_f1":
        return float(summary.get("patient_macro_f1", 0.0))
    aliases = {
...
def _cane_selection_components(summary: Dict[str, float]) -> tuple[float, float, float, float]:
    nez_f1 = float(summary.get("patient_macro_nez_f1", 0.0))
    ez_f1 = float(summary.get("patient_macro_ez_f1", 0.0))
    nez_auprc = float(summary.get("patient_macro_auprc_nez", 0.0))
    ez_auprc = float(summary.get("patient_macro_auprc_ez", 0.0))
    auprc_hmean = 2.0 * nez_auprc * ez_auprc / (nez_auprc + ez_auprc + 1e-8)
    return (
        min(nez_f1, ez_f1),
        float(summary.get("patient_macro_balanced_accuracy", 0.0)),
        auprc_hmean,
        -float(summary.get("predicted_nez_count_mae", float("inf"))),
    )


def _cane_checkpoint_is_better(
    ...
    *,
    ...
    """Apply the documented validation-only CANE tie-break order."""
    if incumbent is None:
        return True
    candidate_f1 = float(candidate.get("patient_macro_f1", 0.0))
    incumbent_f1 = float(incumbent.get("patient_macro_f1", 0.0))
    if candidate_f1 > incumbent_f1 + 1e-6:
        return True
    if candidate_f1 < incumbent_f1 - 1e-6:
        return False
    candidate_components = _cane_selection_components(candidate)
    incumbent_components = _cane_selection_components(incumbent)
    for candidate_value, incumbent_value in zip(candidate_components, incumbent_components):
        if candidate_value > incumbent_value + 1e-12:
            return True
        if candidate_value < incumbent_value - 1e-12:
            return False
    return int(candidate_epoch) < int(incumbent_epoch)
...
class Exp_EZHybridLocalization:
    ...
    def __init__(self, args: Any) -> None:
        ...
        cohort_audit = getattr(args, "sensitivity80_cohort_audit", None)
        if cohort_audit is not None:
            output_dir = Path(getattr(args, "output_dir", "outputs"))
            output_dir.mkdir(parents=True, exist_ok=True)
            (output_dir / "sensitivity80_cohort_audit.json").write_text(
        ...
        self.use_cane_set_nez = bool(getattr(args, "use_cane_set_nez", False))
        self.use_cane_path_cp_nez = bool(getattr(args, "use_cane_path_cp_nez", False))
        self.use_p21_v3_asrr_nez = bool(getattr(args, "use_p21_v3_asrr_nez", False))
        self.use_p23_trn_nez = bool(getattr(args, "use_p23_trn_nez", False))
        self.causal_propagation_store = None
        needs_p23_causal = self.use_p23_trn_nez and bool(getattr(args, "p23_use_causal", False))
        needs_p23_legacy_causal = self.use_p23_trn_nez and bool(getattr(args, "use_causal_propagation_residual", False))
        if (self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez or needs_p23_causal or needs_p23_legacy_causal) and (bool(getattr(args, "use_causal_propagation_residual", False)) or needs_p23_causal):
            from neuroez_c.causal_propagation_features import CausalPropagationFeatureStore

            self.causal_propagation_store = CausalPropagationFeatureStore(
                getattr(args, "causal_propagation_cache_path"),
                getattr(args, "causal_cache_audit_path"),
                require_passed=not bool(getattr(args, "dry_run_config_only", False)),
            )
            if self.use_p21_v3_asrr_nez and not self.causal_propagation_store.has_quality_fields:
                raise ValueError("P2.1 requires causal cache quality fields; legacy six-feature cache is insufficient")
            if needs_p23_causal and not self.causal_propagation_store.has_quality_fields:
                raise ValueError("P23 P6 requires causal cache quality fields")
        self.v3_anchor_store = None
        if self.use_p21_v3_asrr_nez and str(getattr(args, "p21_profile", "R5_FULL")).upper() in {
            "R3_V3_ANCHORED", "R4_Q10_MULTI_SEIZURE", "R5_FULL"
        }:
            from neuroez_c.p21_v3_anchor import V3AnchorStore

            anchor_source = (
                getattr(args, "v3_anchor_ledger_path")
                if str(getattr(args, "v3_anchor_mode", "precomputed_oof_screening")) == "precomputed_oof_screening"
                else getattr(args, "v3_anchor_output_root")
            )
            self.v3_anchor_store = V3AnchorStore(
                anchor_source,
                mode=getattr(args, "v3_anchor_mode", "precomputed_oof_screening"),
                score_semantics=getattr(args, "v3_anchor_input_score_semantics", None),
                expected_patients=len(self.patient_index),
                allowed_subjects=list(self.patient_index),
            )
            audit_dir = Path(getattr(args, "output_dir", "outputs")) / "audit"
            self.v3_anchor_store.write_audit(audit_dir / "p21_v3_anchor_summary.json")
        ...
        if bool(getattr(args, "enforce_fixed_all90_protocol", False)):
            ...
            output_dir = Path(getattr(args, "output_dir", "outputs"))
            output_dir.mkdir(parents=True, exist_ok=True)
            protocol_filename = "sensitivity80_protocol_audit.json" if (self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez or self.use_p23_trn_nez) else "fixed_all90_protocol_audit.json"
            with open(output_dir / protocol_filename, "w", encoding="utf-8") as fout:
        ...
        if self.use_cane_set_nez:
            if str(getattr(args, "positive_label", "nez")).lower() != "nez":
                raise ValueError("CANE requires --positive_label nez")
            if self.train_subject_dropout_count != 0:
                raise ValueError("CANE fixed-All90 does not permit train-subject dropout")
            if self.use_n6_dual_view_ema or self.use_a9v8_lcbo or self.use_broad_ez_mil_loss:
                raise ValueError("CANE cannot be combined with N6, LCBO, or broad-EZ MIL")
        if self.use_cane_path_cp_nez:
            if str(getattr(args, "positive_label", "nez")).lower() != "nez":
                raise ValueError("CANE-PATH-CP requires --positive-label nez")
            if self.train_subject_dropout_count != 0:
                raise ValueError("CANE-PATH-CP sensitivity cohort forbids train-only dropout")
            if self.use_n6_dual_view_ema or self.use_a9v8_lcbo or self.use_broad_ez_mil_loss:
                raise ValueError("CANE-PATH-CP cannot be combined with N6, LCBO, or broad-EZ MIL")
        if self.use_p21_v3_asrr_nez:
            if str(getattr(args, "positive_label", "nez")).lower() != "nez":
                raise ValueError("P2.1 requires --positive-label nez")
            if self.train_subject_dropout_count != 0:
                raise ValueError("P2.1 sensitivity cohort forbids train-only dropout")
        if self.use_p23_trn_nez:
            if str(getattr(args, "positive_label", "nez")).lower() != "nez":
                raise ValueError("P23 requires --positive-label nez")
            if self.train_subject_dropout_count != 0:
                raise ValueError("P23 sensitivity cohort forbids train-only dropout")
            if self.use_n6_dual_view_ema or self.use_a9v8_lcbo or self.use_broad_ez_mil_loss:
                raise ValueError("P23 cannot be combined with N6, LCBO, or broad-EZ MIL")
    ...
    def _build_datasets(
        ...
        *,
        v3_contexts: dict[str, tuple[int, str]] | None = None,
    ) -> Tuple[Any, Any, Any, Any]:
        fit_key = tuple(sorted(map(str, fit_subjects)))
        val_key = tuple(sorted(map(str, val_subjects)))
        test_key = tuple(sorted(map(str, test_subjects)))
        fit_samples = flatten_window_samples(self.run_records, subject_ids=fit_subjects)
        val_samples = fit_samples if val_key == fit_key else flatten_window_samples(self.run_records, subject_ids=val_subjects)
        if test_key == fit_key:
            test_samples = fit_samples
        elif test_key == val_key:
        ...
        if self.use_cane_path_cp_nez or self.use_p21_v3_asrr_nez or self.use_p23_trn_nez:
            build_kwargs["causal_propagation_store"] = self.causal_propagation_store
        def kwargs_for(split_name: str) -> dict[str, Any]:
            result = dict(build_kwargs)
            if self.v3_anchor_store is not None:
                if self.v3_anchor_store.mode == "nested_fold_safe":
                    if not v3_contexts or split_name not in v3_contexts:
                        raise ValueError("Strict P2.1 dataset build requires explicit V3 outer-fold/split-role context")
                    result["v3_anchor_store"] = self.v3_anchor_store.for_context(*v3_contexts[split_name])
                else:
                    result["v3_anchor_store"] = self.v3_anchor_store
            return result
        fit_examples = self.runtime["build_patient_examples"](fit_samples, self.patient_index, normalizer=normalizer, subject_ids=fit_subjects, args=self.args, **kwargs_for("fit"))
        val_examples = fit_examples if val_key == fit_key else self.runtime["build_patient_examples"](
            val_samples, self.patient_index, normalizer=normalizer, subject_ids=val_subjects, args=self.args, **kwargs_for("val")
        )
        if test_key == fit_key:
            test_examples = fit_examples
        elif test_key == val_key:
            test_examples = val_examples
        else:
            test_examples = self.runtime["build_patient_examples"](
                test_samples, self.patient_index, normalizer=normalizer,
                subject_ids=test_subjects, args=self.args, **kwargs_for("test")
            )
        if not val_examples:
    ...
    def _compute_loss(
        ...
        *,
        ...
        if bool(getattr(self.args, "use_p2_scope_v2", False)):
            from neuroez_c.cane_path_cp_loss import compute_cane_path_cp_ranking_loss
            from neuroez_c.p2_scope_v2_loss import scope_losses
            base, parts = compute_cane_path_cp_ranking_loss(outputs, batch, self.args, epoch=int(self.current_epoch))
            extra, scope_parts = scope_losses(outputs, batch, int(self.current_epoch), self.args)
            parts.update(scope_parts); parts["original_p2_total_loss"] = float(base.detach().cpu())
            return base + extra, parts
        if bool(getattr(self, "use_p23_trn_nez", False)) and bool(getattr(self.args, "p23_use_p2_loss", False)):
            from neuroez_c.cane_path_cp_loss import compute_cane_path_cp_ranking_loss
            return compute_cane_path_cp_ranking_loss(outputs, batch, self.args, epoch=int(self.current_epoch))
        if bool(getattr(self, "use_p23_trn_nez", False)):
            from neuroez_c.p23_noisy_ez_loss import compute_p23_loss
            return compute_p23_loss(outputs, batch, self.args, epoch=int(self.current_epoch), teacher_score_nez=None)
        if bool(getattr(self, "use_p21_v3_asrr_nez", False)):
            from neuroez_c.p21_loss import compute_p21_loss

            return compute_p21_loss(outputs, batch, self.args, epoch=int(self.current_epoch))
        if bool(getattr(self, "use_cane_path_cp_nez", False)):
            from neuroez_c.cane_path_cp_loss import compute_cane_path_cp_ranking_loss

            return compute_cane_path_cp_ranking_loss(outputs, batch, self.args, epoch=int(self.current_epoch))
        if bool(getattr(self, "use_cane_set_nez", False)):
            loss, parts = compute_cane_set_loss(outputs, batch, self.args, epoch=int(self.current_epoch))
            return loss, parts
        if bool(getattr(self, "use_n6_dual_view_ema", False)):
    ...
    def _evaluate(
        ...
        *,
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                for key in (
                    ...
                    "direct_score_nez", "anchor_residual", "anchor_distance", "anchor_distance_z",
                    "anchor_nez_evidence", "seizure_residual", "seizure_nez_probability_mean",
                    "seizure_nez_probability_std", "seizure_nez_agreement", "direct_nez_logits",
                    "final_nez_logits",
                ):
                ...
                cane_patient_arrays: Dict[str, np.ndarray] = {}
                if self.use_cane_set_nez:
                    for key in (
                        ...
                        "cardinality_alpha",
                        "cardinality_beta",
                        "cardinality_concentration",
                    ):
                        value = pred_outputs.get(key)
                        if not torch.is_tensor(value) or value.ndim != 1:
                            raise RuntimeError(f"CANE output is missing per-patient field {key}")
                        cane_patient_arrays[key] = value.detach().cpu().numpy()
                    cane_pred_nez = pred_outputs["predicted_nez_mask"].detach().cpu().numpy().astype(bool)
                    cane_pred_ez = pred_outputs["predicted_ez_mask"].detach().cpu().numpy().astype(bool)
                else:
                    cane_pred_nez = None
                    cane_pred_ez = None
                ...
                for idx, subject_id in enumerate(batch["subject_id"]):
                    ...
                    if self.use_cane_set_nez:
                        record.update({
                            ...
                            for key, values in cane_patient_arrays.items()
                        })
                        record.update({
                            "predicted_nez_mask": cane_pred_nez[idx, :c],
                            "predicted_ez_mask": cane_pred_ez[idx, :c],
        ...
        summary["strategy"] = (
            "predicted_nez_cardinality_topk" if self.use_cane_set_nez else "fixed_probability_threshold"
        )
    ...
    def _save_outputs(self, records: Sequence[Dict[str, Any]], *, fold_idx: int, split_name: str) -> None:
        ...
        for record in records:
            ...
            patient_rows.append(
                {
                    ...
                    "predicted_nez_fraction": float(record.get("predicted_nez_fraction", 0.0)),
                    "expected_nez_count": float(record.get("expected_nez_count", 0.0)),
                    "nez_count_error": float(record.get("nez_count_error", 0.0)),
                    "nez_fraction_error": float(record.get("nez_fraction_error", 0.0)),
                    "cardinality_alpha": float(record.get("cardinality_alpha", float("nan"))),
                    "cardinality_beta": float(record.get("cardinality_beta", float("nan"))),
                    "cardinality_concentration": float(record.get("cardinality_concentration", float("nan"))),
                    "decision_rule": str(record.get("decision_rule", "fixed_probability_threshold")),
                    "threshold_source": str(record.get("threshold_source", "fixed")),
                    "true_count_used_for_prediction": bool(record.get("true_count_used_for_prediction", False)),
            ...
            optional_channel_arrays.update({
                "final_score_nez": score_nez,
                "final_score_ez": score_ez,
            })
            if "direct_nez_logits" in optional_channel_arrays:
                optional_channel_arrays["direct_nez_logit"] = optional_channel_arrays["direct_nez_logits"]
            if "final_nez_logits" in optional_channel_arrays:
                optional_channel_arrays["final_nez_logit"] = optional_channel_arrays["final_nez_logits"]
            ...
            for channel_idx, channel_name in enumerate(record["canonical_channels"]):
                channel_dict = {
                        ...
                        "valid_channel": bool(valid_mask[channel_idx]),
    ...
    def run(self) -> List[Dict[str, Any]]:
        if bool(getattr(self.args, "use_p2_scope_v2", False)):
            from neuroez_c.p2_scope_v2_trainer import run_p2_scope_v2
            return run_p2_scope_v2(self)
        if self.use_p23_trn_nez:
            from neuroez_c.p23_trainer import run_p23_trn
            return run_p23_trn(self)
        if self.use_p21_v3_asrr_nez:
            from neuroez_c.p21_trainer import run_p21_v3_asrr

            return run_p21_v3_asrr(self)
        if self.use_cane_path_cp_nez:
            from neuroez_c.cane_path_cp_trainer import run_cane_path_cp_nested, run_cane_path_cp_outer_only

            if bool(getattr(self.args, "cane_direct_outer_only", False)):
                return run_cane_path_cp_outer_only(self)
            return run_cane_path_cp_nested(self)
        ...
        cane_train_rows: List[Dict[str, Any]] = []
        cane_validation_rows: List[Dict[str, Any]] = []
        cane_prototype_rows: List[Dict[str, Any]] = []
        cane_cardinality_rows: List[Dict[str, Any]] = []
        base_seed = int(
            getattr(self.args, "model_seed", 42)
            if self.use_cane_set_nez
            else getattr(self.args, "random_seed", 42)
        )
        split_seed = int(getattr(self.args, "split_seed", getattr(self.args, "random_seed", 42)))
        ...
        self._log(
            ...
            + ("decision_rule=predicted_nez_cardinality_topk" if self.use_cane_set_nez else f"classification_threshold={float(getattr(self.args, 'classification_threshold', 0.5)):.3f}")
        )
        ...
        for split in outer_splits:
            ...
            if self.use_cane_set_nez:
                head_prefixes = ("clean_nez_anchor", "multi_seizure_evidence", "cardinality_head")
                head_parameters = [
                    parameter
                ...
                optimizer = torch.optim.AdamW(
                    [
                        {"params": backbone_parameters, "lr": float(getattr(self.args, "cane_backbone_lr_stage1", 1e-4)), "group_name": "backbone"},
                        {"params": head_parameters, "lr": float(getattr(self.args, "cane_head_lr", 1e-4)), "group_name": "cane_heads"},
                    ],
                    weight_decay=float(getattr(self.args, "weight_decay", 1e-3)),
                )
            elif self.use_n6_dual_view_ema:
                raw_prefixes = (
            ...
            for epoch in range(1, epochs + 1):
                ...
                if self.use_cane_set_nez:
                    backbone_lr = (
                        float(getattr(self.args, "cane_backbone_lr_stage1", 1e-4))
                        if epoch <= int(getattr(self.args, "cane_stage1_end_epoch", 5))
                        else float(getattr(self.args, "cane_backbone_lr_after_warmup", 2e-5))
                    )
                    for group in optimizer.param_groups:
                        group["lr"] = (
                            float(getattr(self.args, "cane_head_lr", 1e-4))
                            if group.get("group_name") == "cane_heads"
                            else backbone_lr
                        )
                train_metrics = self._train_one_epoch(model, train_loader, optimizer, ez_negative_weight, ema_teacher=ema_teacher)
                if self.use_cane_set_nez and epoch == int(getattr(self.args, "cane_stage1_end_epoch", 5)):
                    prototype_audit = initialize_clean_nez_prototypes(
                        model,
                        train_loader,
                        self.device,
                        int(getattr(self.args, "model_seed", 42)),
                        max_samples=int(getattr(self.args, "cane_prototype_max_samples", 20_000)),
                    )
                    prototype_audit.update({"outer_fold": int(fold_idx), "epoch": int(epoch)})
                    cane_prototype_rows.append(dict(prototype_audit))
                    prototype_path = fold_dir / f"cane_prototype_initialization_fold_{fold_idx}_seed_{int(getattr(self.args, 'model_seed', 42))}.json"
                    with prototype_path.open("w", encoding="utf-8") as fout:
                        json.dump(prototype_audit, fout, indent=2, ensure_ascii=False, sort_keys=True)
                if self.use_cane_set_nez:
                    cane_train_rows.append({
                        "outer_fold": int(fold_idx),
                        "model_seed": int(getattr(self.args, "model_seed", 42)),
                        "epoch": int(epoch),
                        "stage": cane_stage(
                            epoch,
                            int(getattr(self.args, "cane_stage1_end_epoch", 5)),
                            int(getattr(self.args, "cane_stage2_end_epoch", 15)),
                        ),
                        **{
                            f"train_{key.removeprefix('cane_')}": float(value)
                            for key, value in train_metrics.items()
                            if key.startswith("cane_") and isinstance(value, (int, float))
                        },
                    })
                    cane_train_rows[-1].update({
                        "mean_anchor_residual": float(train_metrics.get("cane_mean_anchor_residual", 0.0)),
                        "mean_abs_anchor_residual": float(train_metrics.get("cane_mean_abs_anchor_residual", 0.0)),
                        "anchor_gate": float(train_metrics.get("cane_anchor_gate", 0.0)),
                        "mean_seizure_residual": float(train_metrics.get("cane_mean_seizure_residual", 0.0)),
                        "mean_abs_seizure_residual": float(train_metrics.get("cane_mean_abs_seizure_residual", 0.0)),
                        "mean_seizure_agreement": float(train_metrics.get("cane_mean_seizure_agreement", 0.0)),
                        "mean_predicted_nez_fraction": float(train_metrics.get("cane_mean_predicted_nez_fraction", 0.0)),
                        "mean_expected_nez_count": float(train_metrics.get("cane_mean_expected_nez_count", 0.0)),
                        "mean_true_nez_count": float(train_metrics.get("cane_mean_true_nez_count", 0.0)),
                        **{
                            f"prototype_utilization_{index}": float(train_metrics.get(f"cane_prototype_utilization_{index}", 0.0))
                            for index in range(int(getattr(self.args, "cane_num_nez_prototypes", 4)))
                        },
                    })
                if self.use_a9v8_lcbo:
                    train_row = {
                ...
                if self.use_cane_set_nez:
                    val_classification_threshold = float("nan")
                    val_summary = dict(raw_val_summary)
                    val_records = list(raw_val_records)
                elif self.use_n6_dual_view_ema:
                ...
                if self.use_cane_set_nez:
                    ...
                    cane_validation_rows.append({
                        ...
                        "val_patient_macro_f1": float(val_summary.get("patient_macro_f1", 0.0)),
                        "val_patient_macro_nez_f1": float(val_summary.get("patient_macro_nez_f1", 0.0)),
                        "val_patient_macro_ez_f1": float(val_summary.get("patient_macro_ez_f1", 0.0)),
                        "val_patient_macro_balanced_accuracy": float(val_summary.get("patient_macro_balanced_accuracy", 0.0)),
                        ...
                        "val_nez_count_mae": float(val_summary.get("predicted_nez_count_mae", 0.0)),
                        "val_nez_fraction_mae": float(val_summary.get("predicted_nez_fraction_mae", 0.0)),
                    })
                    cane_train_rows[-1].update({
                        key: value
                        for key, value in cane_validation_rows[-1].items()
                        if key.startswith("val_")
                    })
                    cane_train_rows[-1]["val_selection_score"] = float(
                        _summary_score(val_summary, self.args, val_loss=val_loss)
                    )
                if self.use_n6_dual_view_ema:
                ...
                cane_checkpoint_eligible = (
                    not self.use_cane_set_nez
                    or epoch >= int(getattr(self.args, "cane_stage1_end_epoch", 5))
                )
                improved = cane_checkpoint_eligible and (
                    _cane_checkpoint_is_better(
                        val_summary,
                        best_summary if best_epoch > 0 else None,
                        candidate_epoch=epoch,
                        incumbent_epoch=best_epoch,
                    )
                    if self.use_cane_set_nez
                    else val_score > best_score + 1e-6
                )
                if improved:
                    ...
                    torch.save(
                        {
                            ...
                            "classification_threshold": (
                                None if (self.use_n6_dual_view_ema or self.use_cane_set_nez) else float(best_classification_threshold)
                            ),
                            "threshold_source": (
                                "not_used" if self.use_cane_set_nez else "best_ema_validation_only" if self.use_n6_dual_view_ema else "per_epoch_validation"
                            ),
                            "decision_rule": "predicted_nez_cardinality_topk" if self.use_cane_set_nez else "fixed_probability_threshold",
                            "true_count_used_for_prediction": False,
                ...
                if epoch == 1 or epoch % log_interval == 0 or improved:
                    self._log(
                        ...
                        + ("decision_rule=predicted_nez_cardinality_topk | " if self.use_cane_set_nez else f"val_threshold={val_classification_threshold:.2f} | ")
            ...
            if self.use_cane_set_nez and best_epoch < int(getattr(self.args, "cane_stage1_end_epoch", 5)):
                raise RuntimeError("CANE training ended before a post-prototype checkpoint was selected.")
            ...
            if self.use_cane_set_nez:
                test_summary = dict(raw_test_summary)
                test_records = list(raw_test_records)
            else:
            ...
            test_summary["classification_threshold"] = (
                float("nan") if self.use_cane_set_nez else float(best_classification_threshold)
            )
            if self.use_cane_set_nez:
                test_summary.update({
                    "threshold_source": "not_used",
            ...
            if self.use_cane_set_nez:
                ...
                cane_fold_audit = {
                    ...
                    "validation_test_overlap": sorted(validation_set & test_set),
                    "best_epoch": int(best_epoch),
                    "best_stage": cane_stage(
                        best_epoch,
                        int(getattr(self.args, "cane_stage1_end_epoch", 5)),
                        int(getattr(self.args, "cane_stage2_end_epoch", 15)),
                    ),
                    "best_validation_patient_macro_f1": float(best_summary.get("patient_macro_f1", 0.0)),
                    "best_validation_nez_f1": float(best_summary.get("patient_macro_nez_f1", 0.0)),
                    "best_validation_ez_f1": float(best_summary.get("patient_macro_ez_f1", 0.0)),
                    "best_validation_count_mae": float(best_summary.get("predicted_nez_count_mae", 0.0)),
                    "prototype_initialization_source": "fit_clean_nez_only",
                    "prototype_used_validation": False,
                    "prototype_used_test": False,
                    ...
                    "test_evaluations_before_model_selection": 0,
                    "test_labels_used_for_training": False,
                    "test_labels_used_for_count_prediction": False,
                    "test_labels_used_for_ensemble": False,
                }
                output_dir = Path(getattr(self.args, "output_dir", "outputs"))
                with (output_dir / f"cane_fold_{fold_idx}_seed_{model_seed}_audit.json").open("w", encoding="utf-8") as fout:
                    json.dump(cane_fold_audit, fout, indent=2, ensure_ascii=False, sort_keys=True)
                cane_cardinality_rows.append({
                    "outer_fold": int(fold_idx),
                    "model_seed": model_seed,
                    "n_test_patients": int(len(test_records)),
                    "mean_predicted_nez_fraction": _safe_mean([float(record.get("predicted_nez_fraction", 0.0)) for record in test_records]),
                    "mean_expected_nez_count": _safe_mean([float(record.get("expected_nez_count", 0.0)) for record in test_records]),
                    "mean_predicted_nez_count": _safe_mean([float(record.get("predicted_nez_count", 0.0)) for record in test_records]),
                    "nez_count_mae": float(test_summary.get("predicted_nez_count_mae", 0.0)),
                    "nez_fraction_mae": float(test_summary.get("predicted_nez_fraction_mae", 0.0)),
                })
            if self.use_a9v8_lcbo:
                lcbo_target_match_rows.append(
                    {
        ...
        if all_test_records:
            ...
            for key in (
                ...
                "split_seed",
                "model_seed",
                "cane_num_nez_prototypes",
                "cane_prototype_dim",
                "cane_prototype_temperature",
                "cane_prototype_similarity_margin",
                "cane_max_anchor_residual",
                "cane_max_seizure_residual",
                "cane_soft_f1_weight",
                "cane_nez_anchor_weight",
                "cane_hc_rank_weight",
                "cane_count_weight",
                "cane_residual_l2_weight",
            ...
            if self.use_cane_set_nez:
                pd.DataFrame(cane_train_rows).to_csv(output_dir / "cane_train_loss_by_fold_seed_epoch.csv", index=False)
                pd.DataFrame(cane_validation_rows).to_csv(output_dir / "cane_validation_by_fold_seed_epoch.csv", index=False)
                pd.DataFrame(cane_prototype_rows).to_csv(output_dir / "cane_prototype_initialization_audit.csv", index=False)
                pd.DataFrame(cane_cardinality_rows).to_csv(output_dir / "cane_cardinality_by_fold_seed.csv", index=False)
