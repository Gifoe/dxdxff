...
from module2_preprocessing import inspect_mat_file, load_and_preprocess_mat
from module3_labels_metadata import build_patient_dictionaries, parse_mat_channel_labels
...
def _select_window_phase_mask(
    windows_df: pd.DataFrame,
    window_mode: str,
) -> pd.Series:
    analysis_phase = windows_df["analysis_phase"].astype(str).str.strip().str.lower()
    normalized_mode = str(window_mode).strip().lower()
    if normalized_mode == "ictal":
        return analysis_phase.eq("ictal")
    if normalized_mode == "interictal":
        return analysis_phase.eq("interictal")
    return analysis_phase.isin(["ictal", "interictal"])
...
def extract_run_feature_record(
    ...
    *,
    ...
    ez_annotation_path: Optional[str] = None,
    clinical_info_path: Optional[str] = None,
    ...
    raw = None
    try:
        mat_path = run_info.get("mat_path")
        subject_id = str(run_info.get("subject_id", "unknown"))
        if not mat_path:
            return None
        if not ez_annotation_path or not clinical_info_path:
            raise ValueError("MAT feature extraction requires ez_annotation_path and clinical_info_path.")

        eeg_data, sfreq, ch_names_raw = inspect_mat_file(mat_path)
        patient_ez_dict, _ = build_patient_dictionaries(
            ez_annotation_path,
            clinical_info_path,
        )
        contacts_meta = parse_mat_channel_labels(subject_id, ch_names_raw, patient_ez_dict)
        valid_contacts_meta = contacts_meta.loc[contacts_meta["is_valid"].eq(1)].copy()
        if valid_contacts_meta.empty:
            return None

        raw, picked_channels_norm, data, _ = load_and_preprocess_mat(
            mat_path,
            target_sfreq=target_sfreq,
            bandpass_low=spectral_min_freq,
            bandpass_high=spectral_max_freq,
            keep_channel_indices=valid_contacts_meta["channel_order"].tolist(),
            resolved_channel_names_norm=valid_contacts_meta["channel_name_norm"].tolist(),
            eeg_data=eeg_data,
            sfreq=sfreq,
            ch_names_raw=ch_names_raw,
        )
        ...
        phase_mask = _select_window_phase_mask(windows_df, window_mode)
        ...
        metadata = {
            ...
            if key not in {"mat_path"}
        }
        metadata["window_mode"] = str(window_mode)
