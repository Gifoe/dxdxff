...
from module1_file_discovery import discover_all_mat_files
...
def _cache_signature(args: Any) -> Dict[str, Any]:
    return {
        ...
        "ez_annotation_path": _normalize_cache_value(getattr(args, "ez_annotation_path", None)),
        "clinical_info_path": _normalize_cache_value(getattr(args, "clinical_info_path", None)),
        ...
        "window_mode": str(getattr(args, "window_mode", "mixed")),
    }
...
def _cache_path(args: Any) -> Path:
    ...
    return cache_dir / f"techez_mat_cache_v{CACHE_VERSION}_{cache_key}.pkl"
...
def _extract_run_feature_record_from_payload(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    mat_path = str(run_info.get("mat_path", ""))
    try:
        record = extract_run_feature_record(
            ...
            ez_annotation_path=payload["ez_annotation_path"],
            clinical_info_path=payload["clinical_info_path"],
        )
    except Exception as exc:
        _log(f"Skipping run {run_id} ({mat_path}): {exc}")
        return None
...
def build_or_load_patient_bags(args: Any) -> List[Dict[str, Any]]:
    ...
    if external_cache_path_raw:
        ...
        _log("Skipping MAT discovery and feature extraction because an external cache was provided.")
        return patient_bags
    ...
    _log(f"Discovering MAT runs under: {getattr(args, 'dataset_dir', '.')}")
    _log(f"Using EZ annotation table: {getattr(args, 'ez_annotation_path', None)}")
    _log(f"Using clinical information table: {getattr(args, 'clinical_info_path', None)}")
    if getattr(args, "subject_filter", None):
        _log(f"Subject filter enabled: {getattr(args, 'subject_filter', None)}")
    _log(
        "Data options: "
        f"success_only={bool(getattr(args, 'success_only', False))} | "
        f"feature_device={getattr(args, 'feature_device', 'cpu')} | "
        f"feature_num_workers={int(getattr(args, 'feature_num_workers', 20))}"
    )

    runs_df = discover_all_mat_files(
        getattr(args, "dataset_dir", "."),
        ez_annotation_path=getattr(args, "ez_annotation_path", None),
        clinical_info_path=getattr(args, "clinical_info_path", None),
        subject_filter=getattr(args, "subject_filter", None),
        success_only=bool(getattr(args, "success_only", False)),
    )
    if runs_df.empty:
        raise ValueError("No MAT runs were discovered for TeChEZ.")
    _log(
    ...
    payloads = [
        {
            ...
            "step_sec": float(getattr(args, "step_sec", 5.0)),
            "window_mode": str(getattr(args, "window_mode", "mixed")),
            "ez_annotation_path": str(getattr(args, "ez_annotation_path", "")),
            "clinical_info_path": str(getattr(args, "clinical_info_path", "")),
        }
