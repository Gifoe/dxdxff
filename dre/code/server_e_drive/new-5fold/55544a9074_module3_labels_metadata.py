import pandas as pd
import re
from module2_preprocessing import normalize_channel_name

def parse_contact_topology(channel_name_norm: str):
    """
    Takes a normalized channel name (e.g., LAD1) and returns (contact_group, contact_number).
    """
    match = re.match(r'^([A-Z]+)(\d+)$', channel_name_norm)
    if match:
        contact_group = match.group(1)
        contact_number = int(match.group(2))
    else:
        contact_group = channel_name_norm
        contact_number = None
    return contact_group, contact_number

def parse_channel_labels(channels_path, ez_definition="soz_or_resected"):
    df = pd.read_csv(channels_path, sep='\t')
    
    meta_records = []
    
    seen_norms = set()
    
    for _, row in df.iterrows():
        ch_name_orig = str(row['name'])
        ch_name_norm = normalize_channel_name(ch_name_orig)
        
        if ch_name_norm in seen_norms:
            print(f"Warning: Duplicate normalized channel {ch_name_norm} in {channels_path}.")
            continue
        seen_norms.add(ch_name_norm)
        
        ch_type = str(row['type']).upper()
        status = row.get('status', 'good')
        status_desc = str(row.get('status_description', '')).lower()
        
        is_soz = 1 if 'soz' in status_desc else 0
        is_resected = 1 if 'resect' in status_desc else 0
        
        if ez_definition == "soz_or_resected":
            is_ez = 1 if (is_soz or is_resected) else 0
        elif ez_definition == "soz_only":
            is_ez = 1 if is_soz else 0
        else:
            is_ez = 0
            
        is_valid = 1 if status != 'bad' and ch_type in ['SEEG', 'ECOG', 'STEREOEEG'] else 0
        
        contact_group, contact_number = parse_contact_topology(ch_name_norm)
            
        meta_records.append({
            'channel_name_orig': ch_name_orig,
            'channel_name_norm': ch_name_norm,
            'is_soz': is_soz,
            'is_resected': is_resected,
            'is_ez': is_ez,
            'is_valid': is_valid,
            'type': ch_type,
            'contact_group': contact_group,
            'contact_number': contact_number,
            'channel_order': len(meta_records)
        })
        
    return pd.DataFrame(meta_records)
