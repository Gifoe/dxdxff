...
def assign_analysis_phases(windows_df: pd.DataFrame, run_info) -> pd.DataFrame:
    """
    Build the strict analysis-phase view used downstream.

    Only pure `ictal` and pure `interictal` windows participate in the
    dual-stream comparison. `preictal`, `postictal`, and `transition`
    windows are intentionally excluded to avoid contaminating either stream.
    If an ictal run only yields `unknown_ictal` labels, we conservatively keep
    those usable windows inside the ictal stream.
    """
